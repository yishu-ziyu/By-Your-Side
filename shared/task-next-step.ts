...
export function decideTaskNextStep(snapshot: TaskProgressSnapshot, facts: NextStepFacts = {}): TaskNextStep {
  ...
  if(snapshot.state==='aborted')return decide('stop','cancelled',false,'none');
  if(snapshot.state==='paused')return decide('wait','human_control',false,'none');
  if(snapshot.state==='interrupted')return decide('wait','restart_checkpoint',false,'none');
  const incompleteDelivery=facts.inFlight?'none':'partial';
  if(facts.failureLimit)return decide('ask_user','failure_limit',false,incompleteDelivery);
  if(snapshot.unresolvedEffect)return decide('ask_user','unknown_without_baseline',false,incompleteDelivery);
  ...
  if(unknown.length){
    ...
    return decide(verifiable.length?'verify_unknown':'ask_user',verifiable.length?'unknown_with_baseline':'unknown_without_baseline',
      !unknown.some(resultHasWriteEffect),incompleteDelivery,unknown.map(item=>item.id));
  }
  if(facts.inFlight)return decide('wait','in_flight',true,'none');
  if(snapshot.state==='error')return decide('ask_user','runtime_error',false,'partial');
  ...
  if(facts.readbackRequired)return decide('verify_result','readback_required',true,'partial');
  // A failed read with no registered obligation still cannot turn into a full result.
...
export function nextStepInstruction(decision: TaskNextStep): string {
  switch(decision.reason){
    ...
    case 'remaining':return '继续原任务中的未完成项，不把登记或工具计划当成完成。';
...
export function partialResultNote(decision:TaskNextStep):string {
  const limit=decision.reason.startsWith('unknown_')?'有操作的结果仍无法确认，没有将其记为完成。'
    :decision.reason==='readback_required'?'执行结果尚未完成页面核对。'
    :decision.reason==='failure_limit'||decision.reason==='tool_failed'?'仍有步骤执行失败，未声明全部完成。'
    :'仍有未完成或未核验事项，未声明全部完成。';
...
export function assertTaskStepExecution(snapshot: TaskProgressSnapshot|null, name:string, params:Record<string,unknown>={}, worker=false):void {
  ...
  if(worker&&!write)return;
  const decision=decideTaskNextStep(snapshot,{failureLimit:snapshot.nextStep?.reason==='failure_limit'});
  if(write&&['cancelled','human_control','restart_checkpoint','runtime_error','failure_limit'].includes(decision.reason))throw new Error(nextStepInstruction(decision));
  ...
  for(const item of snapshot.results??[]){
    if(item.status==='unknown'&&!isSupersededUnknown(item,snapshot.results??[])){
      if(!worker&&item.tool===name&&(item.target===null||item.target===target))throw new Error(`「${item.description}」的执行结果未知，不能自动重做；请先查询结果或由用户决定。`);
      if(write&&resultHasWriteEffect(item))throw new Error(`任务中存在尚未确认结果的操作「${item.description}」，当前写入已暂停。请先用 snapshot 或 read_element 观察核查页面，不得盲目重试。`);
    }
    if(!write||item.tool!==name||item.status!=='satisfied'||item.target===null||item.target!==target)continue;
    ...
    if(snapshot.restartRecovery||(!worker&&!observedAfter))throw new Error(snapshot.restartRecovery&&name==='fill'
  ...
  if(write&&!decision.allowWrites)throw new Error(nextStepInstruction(decision));
  if(write&&(snapshot.results?.length??0)>=MAX_TASK_RESULTS){
    ...
    if(!ownsSlot&&binding.kind!=='exact'&&binding.kind!=='rebind')throw new Error('结果账本已满，无法可靠记录新的写入结果；本次操作未执行，请先交付已有结果。');
  }
}
