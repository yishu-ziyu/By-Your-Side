...
const out=resolve('out/experiments',`typesafe-display-${Date.now()}`);
...
assert.equal(model,'opencode-go/deepseek-flash','Reassess comparison if configured model changed');

const results:any[]=[],routing:any[]=[];
...
const songtiOnly=process.argv.includes('--songti-only');
...
try{
 ...
 const call=async(params:any)=>{const r=await iso.tool('page_translation',{tabId:tab.id,...params},'main');assert(r.ok,r.error);
...
return r.data;};
 ...
 async function watchDOM(completed:()=>Promise<boolean>,started:number,signal:AbortSignal){while(!signal.aborted){if(await completed())return performance.now()-started;await sleep(75);}
 ...
 async function baseline(text:string,row:any,completed:()=>Promise<boolean>){
  ...
  manager=new ConversationManager((id,emit,summary)=>createConversationRuntime(id,emit,model,{sessionManager:store.sessionManager(id),mode:summary?.mode}),message=>{
   ...
   if(message.type==='tool_call'){
    ...
    })().catch(async()=>{row.bridgeError=true;await manager?.handleMessage({type:'tool_result',conversationId:message.conversationId,id:message.id,ok:false,error:'Benchmark bridge failed',executionFact:'unknown'} as any);manager?.get('default')?.runtime.session.abort();});
