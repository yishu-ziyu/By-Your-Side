...
const out=resolve('out/experiments',`goal-voice-${Date.now()}`);

await mkdir(out,{recursive:true});

const rows=[];

for(const item of cases){const at=Date.now();let row;
...
}catch(error){row={...item,error:String(error),passed:false};}

rows.push(row);console.log(JSON.stringify(row));}

await writeFile(resolve(out,'results.json'),JSON.stringify({model:pattern,rows},null,2));

console.log(JSON.stringify({out,passed:rows.every(row=>row.passed)}));
