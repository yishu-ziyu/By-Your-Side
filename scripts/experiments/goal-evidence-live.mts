...
const out=resolve('out/experiments',`goal-evidence-live-${Date.now()}`);await mkdir(out,{recursive:true});
...
const rows=[];
for(const c of cases){const at=Date.now();let row;try{const result=await reviewTaskGoal({runtime,model,sessionId:'bys-goal-verification-eval',headers:undefined},c.stage,c.data,AbortSignal.timeout(28000));row={id:c.id,split:c.split,stage:c.stage,expected:c.expected,...result,elapsedMs:Date.now()-at,passed:result.matched===c.expected};}catch(e){row={id:c.id,split:c.split,passed:false,error:String(e)};}
...
await writeFile(resolve(out,'results.json'),JSON.stringify(report,null,2));console.log(JSON.stringify({out,passed:report.passed}));if(!report.passed)process.exitCode=1;
