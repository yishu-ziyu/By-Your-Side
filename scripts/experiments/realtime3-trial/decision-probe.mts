...
for(let i=0;i<args.length;i++){const a=args[i]!;if(!known.has(a))throw new Error(`未知参数 ${a}；${USAGE}`);if(a==='--cases'||a==='--out'||a==='--batch-ms'){const v=args[++i];if(!v||v.startsWith('--'))throw new Error(`${a} 缺值；${USAGE}`);}}
...
const fail=(e:unknown):ArmResult=>{const msg=redact(e);if(batchController.signal.aborted)return {status:'BATCH_TIMEOUT',raw:`batch abort: ${msg}`};return {status:/abort|timeout/i.test(msg)?'TIMEOUT':'PROVIDER_ERROR',raw:msg};};
...
function r3Decide(c:Case,key:string):Promise<ArmResult>{
  return new Promise(resolve=>{
    ...
    const done=(r:ArmResult)=>{if(settled)return;settled=true;clearTimeout(timer);batchController.signal.removeEventListener('abort',onAbort);activeSockets.delete(ws);try{ws.close()}catch{}resolve({...r,elapsedMs:Date.now()-start});};
    ...
    const finish=(rawArgs:string)=>{
      let a:any=null;try{a=JSON.parse(rawArgs)}catch{}
    ...
    ws.on('message',(data:RawData)=>{
      let e:any;try{e=JSON.parse(String(data))}catch{return}
...
async function runArm(arm:string,c:Case,run:()=>Promise<ArmResult>){
  ...
  let r:ArmResult;try{r=await run()}catch(e){r=fail(e)}
...
const finalize=(complete:boolean,reason:string|null=null):Promise<number>=>{
  ...
  const perCase=labels.cases.map(c=>({caseId:c.id,expected:c.expected,records:ARMS.map(a=>{const r=results.find(x=>x.arm===a&&x.caseId===c.id);return r?{arm:a,status:r.status,action:r.action,target:r.target,typedOk:r.typedOk,actionMatch:r.actionMatch,targetMatch:r.targetMatch,elapsedMs:r.elapsedMs,raw:r.raw}:{arm:a,status:'MISSING'}})}));
