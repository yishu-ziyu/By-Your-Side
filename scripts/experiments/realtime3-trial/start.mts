...
let previousReady:any;try{previousReady=JSON.parse(await readFile(join(out,'ready.json'),'utf8'));}catch{}
...
const http=createServer((req,res)=>{
  if(req.method==='GET'&&req.url==='/health'){res.setHeader('Content-Type','application/json');res.end(JSON.stringify({ready:!!swSession&&host.socket?.readyState===WebSocket.OPEN,model:'stepaudio-3-realtime-preview',headless,hostConnected:host.socket?.readyState===WebSocket.OPEN}));return;}
...
host.wss.on('connection',client=>{
  ...
  client.on('message',raw=>{try{const m=JSON.parse(String(raw));if(m.type==='hello')log({type:'task_hello',tokenAccepted:m.token===host.token});}catch{}});
  ...
  (client as any).send=(data:any,...args:any[])=>{
    let m:any; try{m=JSON.parse(String(data));}catch{return (original as any)(data,...args);}
...
wss.on('connection',(client,req)=>{
  if(req.url!=='/voice'||req.headers.origin!==`chrome-extension://${extensionId}`){client.close(1008,'origin rejected');return;}
  let authenticated=false;
  const timer=setTimeout(()=>client.close(1008,'auth timeout'),3000);
  client.on('message',raw=>{
    let message:Record<string,unknown>;try{message=JSON.parse(String(raw));}catch{client.close(1008,'invalid frame');return;}
    if(!authenticated){
      if(message.type!=='auth'||message.token!==token||voiceClient?.readyState===WebSocket.OPEN){client.close(1008,'auth rejected or another call active');return;}
      ...
      const session=new TrialVoiceSession({key,send:event=>{if(client.readyState===WebSocket.OPEN)client.send(JSON.stringify(event));},log,tools:{
        read_page:async()=>{const context=await page();const text=await swEval(`chrome.scripting.executeScript({target:{tabId:${context.tabId}},func:()=>({text:document.body.innerText.slice(0,18000),selection:getSelection()?.toString().slice(0,5000)||''})}).then(r=>r[0]?.result)`);return {context,...text};},
      ...
      voice=session;session.start();return;
    }
    if(voiceClient===client) voice?.handle(message);
  });
  client.on('close',()=>{clearTimeout(timer);if(voiceClient===client){voice?.close();voice=undefined;voiceClient=undefined;}});
});
async function until<T>(fn:()=>Promise<T|undefined>,label:string,ms=20000):Promise<T>{const end=Date.now()+ms;while(Date.now()<end){try{const x=await fn();if(x!==undefined&&x!==false)return x;}catch{}await new Promise(r=>setTimeout(r,150));}throw new Error(`等待超时：${label}`);}
async function shutdown(exitCode=0){if(closing)return;closing=true;voice?.close();for(const c of wss.clients)c.terminate();await stopHost(host).catch(()=>{});browser?.kill('SIGTERM');await cdp?.close().catch(()=>{});http.close();wss.close();await writeFile(join(out,'stopped.json'),JSON.stringify({at:Date.now(),pid:process.pid,exitCode}));let last:any;try{last=JSON.parse(await readFile(join(out,'ready.json'),'utf8'));}catch{}await writeFile(join(out,'ready.json'),JSON.stringify({...last,pid:process.pid,out,ready:false,phase:exitCode?'failed':'stopped'}),{mode:0o600});setTimeout(()=>process.exit(exitCode),300).unref();}
...
try{
  ...
  if(previous?.out===out&&/^[a-p]{32}$/.test(previous.extensionId??'')){
    ...
    await until(async()=>{const r=await cdp!.send('Runtime.evaluate',{expression:`!!globalThis.chrome?.developerPrivate?.reload`,returnByValue:true},managementSession);return r.result?.value||undefined;},'试用扩展重载入口');
  ...
  if(headless){panelId=(await cdp.send('Target.createTarget',{url:`chrome-extension://${extensionId}/${panelFile}`})).targetId;}
    ...
    const point=await until(async()=>{const r=await cdp!.send('Runtime.evaluate',{expression:`(()=>{const b=document.querySelector('#open');if(!b||b.disabled)return null;const r=b.getBoundingClientRect();return {x:r.x+r.width/2,y:r.y+r.height/2}})()`,returnByValue:true},openerSession);return r.result?.value??undefined;},'打开按钮');
    ...
    panelId=(await until(async()=>{const {targetInfos}=await cdp!.send('Target.getTargets');return targetInfos.find((t:any)=>t.url===`chrome-extension://${extensionId}/${panelFile}`);},'试用侧栏')).targetId;
  }
  const panelSession=await cdp.attachSession(panelId);
  await until(async()=>{const r=await cdp!.send('Runtime.evaluate',{expression:'!!globalThis.__r3Trial',returnByValue:true},panelSession);return r.result?.value||undefined;},'语音试用控件');
...
}catch(error){if(cdp)try{const t=await cdp.send('Target.createTarget',{url:'chrome://extensions/'});const s=await cdp.attachSession(t.targetId);await until(async()=>{const r=await cdp!.send('Runtime.evaluate',{expression:'!!globalThis.chrome?.developerPrivate?.getExtensionsInfo',returnByValue:true},s);return r.result?.value||undefined;},'扩展错误读取',3000);const r=await cdp.send('Runtime.evaluate',{expression:`chrome.developerPrivate.getExtensionsInfo({includeDisabled:true,includeTerminated:true}).then(xs=>xs.filter(x=>x.name.includes('Realtime')).map(x=>({id:x.id,state:x.state,disableReasons:x.disableReasons,manifestErrors:x.manifestErrors,runtimeErrors:x.runtimeErrors})))`,returnByValue:true,awaitPromise:true},s);log({type:'extension_startup_errors',details:r.result?.value});}catch{}if(swSession)try{log({type:'startup_connection_diagnostic',expectedPort:host.port,connections:await swEval('globalThis.__trialConnections')});}catch{}log({type:'startup_failed',message:String(error)});console.error(String(error).replaceAll(key,'[redacted]'));process.exitCode=1;await shutdown(1);}
