...
async function jsonBody(request:IncomingMessage):Promise<any>{
  let body='';for await(const chunk of request){body+=chunk.toString();if(body.length>16000)throw new Error('payload too large');}
  ...
  return createServer(async(request,response)=>{
    ...
    try{
      if(request.method==='POST'){
        if(request.headers.origin&&request.headers.origin!==`http://${request.headers.host}`){json({error:'same-origin fixture only'},403);return;}
        const body=await jsonBody(request);
        if(url.pathname==='/api/fault'){dropNextReceipt=body.dropNextReceipt===true;json({armed:dropNextReceipt});return;}
        if(url.pathname==='/api/account'){if(!['A','B'].includes(body.account))throw new Error('account must be A or B');account=body.account;json({account});return;}
      ...
      if(request.method!=='GET'){json({error:'method not allowed'},405);return;}
      ...
      else if(url.pathname.startsWith('/offer/')){const offer=offers.find(o=>url.pathname===`/offer/${o.id}`);if(!offer){json({error:'not found'},404);return;}body=`<h1>${offer.name}方案</h1><p>价格：${offer.price} 元</p><p>退换：${offer.returns?'支持七天退换':'不支持退换'}</p><a href="/research">返回比较</a>`;}
...
form.addEventListener('submit',async event=>{event.preventDefault();const input={name:document.querySelector('#name').value,choice:document.querySelector('#choice').value,page:location.pathname};try{const response=await fetch('/api/submit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(input)});if(!response.ok)throw Error('request failed');await response.json();await refresh();}catch{receipt.textContent='保存回执丢失，结果未确认，请勿重复保存。';}});refresh();
      ...
      else{json({error:'not found'},404);return;}
