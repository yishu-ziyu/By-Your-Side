/** Third-round independent evidence-scope counterexamples: hidden old content, unrelated update, changed document. Only fault and RPC wiring vary.
...
const mode=process.argv.find(a=>a.startsWith('--case='))?.slice(7)??'hidden';

if(!['hidden','background','navigation'].includes(mode))throw Error('unknown case');

const out=await mkdtemp(join(tmpdir(),'ego-write-receipt-scope-'+mode+'-'));
...
while(Date.now()<end){const v=await fn();

if(v)return v;await sleep(100);}

throw Error('Evaluator prerequisite timed out');}
...
const server=createServer(async(req,res)=>{
 ...
 if(req.method==='POST'&&u.pathname==='/append'){
  ...
  res.setHeader('Content-Type','application/json');res.end(JSON.stringify(list));

return;
 }

 if(u.pathname!=='/'){res.statusCode=404;res.end();

return;}

 res.setHeader('Content-Type','text/html; charset=utf-8');
 res.end(`<!doctype html><meta charset="utf-8"><title>回执丢失验收</title><style>body{font:22px sans-serif;padding:50px}button{padding:20px}</style><h1>${u.searchParams.has("after")?"另一文档证明":"隔离记录页"}</h1><div id="old" hidden>历史订单731</div><p id="banner">暂无通知</p><button id="append">新增一条记录</button><output id="count">0</output><ul id="rows"></ul><script>document.querySelector('#append').onclick=async()=>{const rows=await fetch('/append?case=${key}',{method:'POST'}).then(r=>r.json());document.querySelector('#count').textContent=String(rows.length);document.querySelector('#rows').innerHTML=rows.map(r=>'<li>记录 '+r.id+'</li>').join('');};</script>`);
});
...
try{
 ...
 const port=await until(async()=>{if(launchError)throw launchError;
...
try{return(await readFile(join(profile,'DevToolsActivePort'),'utf8')).split('\n')[0];}catch{return undefined;}});
 ...
 const sid=await until(async()=>{const r=await cdp!.send('Target.getTargets');const t=r.targetInfos.find((t:any)=>t.type==='service_worker'&&t.url.startsWith('chrome-extension://'));

return t?await cdp!.attachSession(t.targetId):undefined;});
...
return r.result?.value;};
 ...
 for(const fault of ['unknown_error']){
  ...
  const raw:any={subscribe:(fn:any)=>{emitSdk=fn;
...
return()=>{};}};
  ...
  async function invoke(name:string,id:string,params:any){
   ...
   try{const result=await tools.find(t=>t.name===name)!.execute(id,params);emitSdk({type:'tool_execution_end',toolCallId:id,toolName:name,isError:false,result});

return {ok:true};}
   catch(e){const error=String(e);emitSdk({type:'tool_execution_end',toolCallId:id,toolName:name,isError:true,result:{content:[{type:'text',text:error}]}});
...
if(bridgeError)throw bridgeError;
  ...
  if(fault==='normal'){
   ...
   const baseline=record.events.find((e:any)=>e.kind==='tool_observation'&&e.name==='snapshot');
   check('full baseline observation is present',!!baseline&&!baseline.truncated);
   let target='#old',expect='历史订单731';

   if(mode==='hidden'){
    check('old hidden text absent from AX baseline',!baseline.text.includes(expect));
    record.hiddenExists=await evaluate(`chrome.scripting.executeScript({target:{tabId:${tab.id}},func:()=>document.querySelector('#old').textContent}).then(r=>r[0].result)`);
    check('old hidden content really existed in initial fixture',record.hiddenExists===expect);
   }else if(mode==='background'){
    target='#banner';expect='背景刷新812';
    // Independent environment change, not the outcome of the append action.
    await evaluate(`chrome.scripting.executeScript({target:{tabId:${tab.id}},func:()=>document.querySelector('#banner').textContent='背景刷新812'})`);
   }else{
    target='h1';expect='另一文档证明';
    record.beforeDocument=await evaluate(`chrome.scripting.executeScript({target:{tabId:${tab.id}},func:()=>location.href}).then(r=>({id:r[0].documentId,url:r[0].result}))`);
    await evaluate(`chrome.tabs.update(${tab.id},{url:${JSON.stringify(url+'?case='+fault+'&after=1')}})`);
    await until(async()=>await evaluate(`chrome.scripting.executeScript({target:{tabId:${tab.id}},func:()=>document.querySelector('h1')?.textContent==='另一文档证明'}).then(r=>r[0].result)`)||undefined);
    record.afterDocument=await evaluate(`chrome.scripting.executeScript({target:{tabId:${tab.id}},func:()=>location.href}).then(r=>({id:r[0].documentId,url:r[0].result}))`);
    check('same tab now contains a different document',record.beforeDocument.id!==record.afterDocument.id);
   }

   record.verification=await verifier.execute('verify-unrelated-evidence',{id:intent.id,target,expect});
   record.afterVerification=progress.snapshot();
   check(mode+' evidence cannot resolve this write unknown',record.afterVerification.resultState==='unknown');
