...
for(const name of ['静音','100ms噪声','连续白噪声','短冲击声']){
 ...
 for(let i=0;i<pcm.length/2;i++){const t=i/24000;let x=0;

if(name==='100ms噪声'&&t>=.5&&t<.6)x=random()*.04;

if(name==='连续白噪声')x=random()*.04;

if(name==='短冲击声'&&i%12000<120)x=random()*.4;pcm.writeInt16LE(Math.round(x*32767),i*2)}
...
try{
 if(p==='/'){res.setHeader('Content-Type','text/html; charset=utf-8');res.end('<!doctype html><meta charset="utf-8"><title>By Your Side · 人声检测实测</title><style>body{font:18px/2 system-ui;max-width:850px;margin:50px auto;padding:20px}li{margin:12px 0}</style><h1>人声检测实测</h1><p>正在使用本地模型检测合成中文和噪声样本，不使用你的麦克风。</p><ul></ul><script type="module" src="check.js"></script>');

return}

 const file=resolve(p==='/check.js'||p.endsWith('.pcm')?out:'extension/dist','.'+p);

if(!file.startsWith(resolve(out)+'/')&&!file.startsWith(resolve('extension/dist')+'/'))throw Error('path');res.setHeader('Content-Type',({'.js':'text/javascript','.mjs':'text/javascript','.wasm':'application/wasm'} as any)[extname(file)]??'application/octet-stream');res.end(await readFile(file));
...
const cdp=createCdp(info.webSocketDebuggerUrl);
...
try{
 ...
 for(let i=0;i<30;i++){const list=await cdp.send('Target.getTargets');target=list.targetInfos.find((t:any)=>t.url===pageUrl);

if(target)break;await new Promise(r=>setTimeout(r,100))}
...
await cdp.send('Runtime.enable',{},sid);cdp.onEvent('Runtime.exceptionThrown',(e:any)=>{if(e.sessionId===sid)console.log('BROWSER ERROR',e.params.exceptionDetails.text,e.params.exceptionDetails.exception?.description)});console.log(JSON.stringify({out,url,target:t.targetId}));
 ...
 const deadline=Date.now()+100000;let done=false;

while(Date.now()<deadline){const r=await cdp.send('Runtime.evaluate',{expression:'JSON.stringify({done:globalThis.done,failure:globalThis.failure,results:globalThis.results})',returnByValue:true},sid);const data=JSON.parse(r.result.value);

if(data.failure)throw Error(data.failure);

if(data.done){await writeFile(join(out,'results.json'),JSON.stringify(data,null,2));console.log(JSON.stringify(data));done=true;process.exitCode=data.results.every((r:any)=>r.ok)?0:1;break}

await new Promise(r=>setTimeout(r,1000))}

if(!done)throw Error('Visible speech check timed out');
