...
async function until(test:()=>boolean,ms:number,label:string){const end=Date.now()+ms;

while(!test()){if(Date.now()>end)throw Error(label);await sleep(100);}}
...
finally{await connection?.close();await write('report',report);console.log(JSON.stringify({status:report.status,out,results:report.results.map((r:any)=>({mode:r.mode,ok:r.ok,error:r.error,checks:r.checks})),error:report.error}));
...
if(gate&&!gate.writableEnded)gate.end('continue');};

 const server=createServer((req,res)=>{
  if(req.url==='/gate'){gate=res;

if(released)res.end('continue');

return;}

  if(req.url?.startsWith('/applied?')){evidence.applications.push({at:Date.now(),...JSON.parse(decodeURIComponent(req.url.slice(9)))});res.end('ok');
...
if(owned.length!==1)throw Error(`Expected one fixture tab, got ${owned.length}`);
