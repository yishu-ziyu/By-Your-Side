...
const out=resolve('out/acceptance/20260920-voice-read-repair');await mkdir(out,{recursive:true});
...
const events:any[]=[],diagnostics:any[]=[],reads:any[]=[];let host:any,iso:any,voice:any,panel:string|undefined;
...
const wait=async(fn:()=>Promise<any>|any,label:string,ms=25000)=>{const end=Date.now()+ms;while(Date.now()<end){const r=await fn();if(r)return r;await new Promise(r=>setTimeout(r,80));}throw new Error(label+' timeout');};
try{
 ...
 voice=new VoiceService((id:string)=>host.manager.getTaskProgress(id),(msg:any)=>{events.push({at:Date.now(),direction:'server',message:msg});if(host.socket?.readyState===WebSocket.OPEN)host.socket.send(JSON.stringify(msg));},undefined,undefined,undefined,
  ...
  async(id:string,input:any)=>{const start=Date.now();try{const result=await readVoicePage(host.manager.get(id).runtime.rpc,input);reads.push({at:start,ms:Date.now()-start,ok:true,result});return result;}catch(e){reads.push({at:start,ms:Date.now()-start,ok:false,error:String(e)});throw e;}});
 ...
 for(let i=0;i<questions.length;i++){
  ...
  await wait(()=>{
   ...
   const done=diagnostics.filter(d=>d.at>(output?.at??start)&&d.event==='response_done').at(-1);if(!done)return false;
 ...
 await save();console.log(JSON.stringify({passed:report.passed,error:report.error,checks:report.checks,reads:reads.map(r=>({ok:r.ok,ms:r.ms,error:r.error}))},null,2));if(!report.passed)process.exitCode=1;
}
