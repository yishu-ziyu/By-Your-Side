...
const out=`/tmp/ego-voice-intents-${Date.now()}`;
...
const cases=rows.flatMap(row=>repeats.map(repeat=>({row,repeat})));
...
try{
 ...
 console.log(JSON.stringify({out,ok:report.ok,passed:report.results.filter((r:any)=>r.ok).length,total:cases.length,error:report.error}));
