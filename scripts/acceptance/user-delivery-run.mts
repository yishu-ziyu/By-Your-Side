...
return;}
 ...
 const invitations=[`<li><a href="/activity">${eventName}</a></li>`,`<li><a href="/interview">${interviewName}</a></li>`];

if(first%2)invitations.reverse();
...
if(observation)m.command={...m.command,input:{...m.command.input,observation}};}
...
return manager!.handleMessage({type:'tool_result',conversationId:message.conversationId,id:message.id,ok:true,data});},error=>{record.ok=false;record.error=String(error);
...
for(const m of bufferedPanel.splice(0))postPanel(m);await panelQueue;
 ...
 const speak=async(text:string,_input?:VoiceInputContext)=>{
  ...
  const pcm=Buffer.concat([await readFile(base+'.pcm'),Buffer.alloc(48000)]);const pageList=await evaluate(`probe.execute(${JSON.stringify({name:'list_tabs',params:{},conversationId:'default',sessionId:'main'})})`).catch(()=>null);const activePage=pageList?.tabs?.find((t:any)=>t.working&&t.url.startsWith(url)&&!t.url.endsWith('sidepanel.html'));

if(activePage?.id)await evaluate(`chrome.tabs.update(${JSON.stringify(activePage.id)},{active:true})`);await panelEval!(`injectSpeech(${JSON.stringify(pcm.toString('base64'))})`);
  await until(()=>{if(report.transportError)throw Error(report.transportError);const events=messages.slice(begin).filter(m=>m.type==='voice'&&m.voiceId===voiceId);const err=events.find(m=>m.event.kind==='state'&&m.event.state==='error');

if(err)throw Error(err.event.detail);

return events.find(m=>m.event.kind==='response_end');},60000);
