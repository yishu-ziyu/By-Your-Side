...
export async function until(check, description, timeout = 30000) {
  ...
  while (Date.now() < deadline) { const result = await check();

 if (result) return result; await pause(100); }
...
async function run() {
  ...
  try {
    ...
    const held=await until(async()=>{const s=await gate();

return s.gate.user&&!s.gate.draining?s:false;},'takeover confirmed',60000);
    ...
    function findButton(node){if(node.nodeName==='BUTTON'&&(node.children||[]).some(c=>c.nodeValue==='交还'))return node;

for(const child of [...(node.children||[]),...(node.shadowRoots||[])]){const found=findButton(child);
...
if(found)return found;}}
    ...
    await until(async()=>{const s=await gate();

return !s.gate.user&&!s.gate.draining?s:false;},'handback confirmed',60000);
    ...
    result.after=await until(async()=>{const state=await readViaContent();

if(state.submitted)throw Error('Unexpected fixture submission');

return state.draft===result.draft?state:false;},'native Agent resumes original draft',240000);
