...
const out=resolve('out/acceptance',`voice-input-order-${Date.now()}`);await mkdir(out,{recursive:true});
class Asr extends EventEmitter {
 readyState=1;bufferedAmount=0;sent:any[]=[];commits=0;
 send(raw:string){const e=JSON.parse(raw);this.sent.push(e);if(e.type==='input_audio_buffer.commit'){
...
const voice=new StepVoiceSession({earlyReplies:true,getSnapshot:()=>manager?.getTaskProgress('default')??({conversationId:'default',observedAt:Date.now(),state:'idle',goal:null,startedAt:null,runId:null,controlVersion:0,active:[],lastAction:null,successVerified:false}),connect:()=>{if(!liveAsr)return asr as any;const ws=new WebSocket(STEP_VOICE_ENDPOINT,{headers:{Authorization:`Bearer ${key}`},handshakeTimeout:12000});ws.on('message',raw=>{try{providerEvents.push(JSON.parse(raw.toString()).type)}catch{}});return ws;},emit:event=>socket?.send(JSON.stringify({type:'voice',voiceId,conversationId:'default',event})),route:async(text,start,current,context)=>{routes.push({text,input:context.input});if(manager)return manager.routeVoiceInput('default',text,start,current,context);context.onInputDecision?.(true);return {kind:'silent'}}});
...
server.on('connection',ws=>ws.on('message',raw=>{
 const m=parseClientMessage(raw.toString());if(!m)return;
 if(m.type==='hello'){if(m.token!==token){ws.close();return;}socket=ws;ws.send(JSON.stringify({type:'hello_ok',version:PROTOCOL_VERSION,model:'fixture',models:[],hostVersion:HOST_VERSION,storageSchema:STORAGE_SCHEMA_VERSION,extensionVersion:'0.1.0'}));return;}
 if(m.type!=='voice'){void manager?.handleMessage(m);return;}voiceId=m.voiceId;commands.push({at:Date.now(),command:m.command});
 if(m.command.kind==='start'){voice.start(key);if(!liveAsr){asr.server({type:'session.created',session:{model:'stepaudio-2.5-realtime'}});asr.server({type:'session.updated',session:{voice:STEP_VOICE,input_audio_format:'pcm16',turn_detection:null}});}}else voice.command(m.command);
}));
let ok=false;
try{
 await listening;if(manager)await manager.ensureDefault();iso=await launchIsolatedExtension({fixtureHtml:'<!doctype html><title>Input order fixture</title><h1>Viewing room</h1><p>The viewing room opens at 10:30 every morning. A ticket costs 35 yuan.</p>'});
 ...
 voice.close();manager?.dispose();await iso?.close();for(const client of server.clients)client.terminate();await new Promise<void>(r=>server.close(()=>r()));
}
