...
const out=resolve('docs/evals/20260917-page-translation-reliability/live');await mkdir(out,{recursive:true});
...
try{
 ...
 const articleUrl='https://www.openhands.dev/';
 ...
 await until(async()=>await page("document.readyState==='complete' && !!document.querySelector('h1')")||undefined,30000,'original article ready');
 ...
 assert.equal(state.mode,'translated');assert(state.translated>70);
 // Animated demo may replace text while translation runs; retain honest remaining counts.
 const generatedBefore=events.filter(e=>e.message?.type==='tool_call'&&e.message.params?.action==='apply').length;
 const fontCalls=await send('字体改成宋体。');assert(fontCalls.some(c=>c.name==='page_translation'&&c.params.action==='display'&&c.params.fontFamily==='songti'));
 assert.equal(events.filter(e=>e.message?.type==='tool_call'&&e.message.params?.action==='apply').length,generatedBefore);
 assert.match(await page('getComputedStyle(document.querySelector("h1")).fontFamily'),/Songti SC/);
 assert.match(await page('document.querySelector("h1").textContent'),/[\u4e00-\u9fff]/);
 ...
 await writeFile(join(out,'results.json'),JSON.stringify({passed:true,url:articleUrl,model:entry.runtime.session.modelName(),injectedFailures:0,failures,state,cases},null,2));
 console.log('PASS: OpenHands translation and Songti through actual panel and configured model');
}finally{
