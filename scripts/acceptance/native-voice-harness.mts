...
export class NativeVoiceHarness {
  ...
  async w(expression:string):Promise<any>{new Function(expression);

return evaluateInWorker(this.connection.cdp,this.connection.sid,expression);}
  async p(expression:string,timeoutMs=30000):Promise<any>{new Function(expression);const r=await this.connection.cdp.send('Runtime.evaluate',{expression,awaitPromise:true,returnByValue:true},this.pageSid,timeoutMs);
...
return r.result?.value;}
  ...
  async wait(expression:string,ms=60000):Promise<any>{const end=Date.now()+ms;

while(Date.now()<end){const result=await this.w(expression);

if(result)return result;await sleep(100);}
...
throw Error(`Timed out: ${expression.slice(0,100)}`);}
  ...
  check(name:string,ok:boolean){this.checks.push({name,ok});console.log(`${ok?'PASS':'FAIL'} ${name}`);
...
if(!ok)throw Error(name);}
  async create(title:string):Promise<string>{
    ...
    await this.wait(`${this.events(id)}.some(e=>e.type==='model_info'&&e.model==='minimax-cn/MiniMax-M3')`);
...
return id;
  ...
  async speak(text:string,input?:VoiceInputContext,quiet=false){
    ...
    else {const response=await this.wait(`${source}.find(e=>e.event.kind==='response_end'&&e.event.turn===${turn}||e.event.kind==='state'&&e.event.state==='error')`,55000);

if(response.event.kind==='state')throw Error(response.event.detail);}

    return turn;
  }
  async openTab(id:string,url:string,worker='main'){
    const r=await this.connection.tool(id,worker,'open_tab',{url});

if(!r.ok)throw Error(r.error);this.tabs.add(r.data.tabId);
  ...
  async close(){
    ...
    for(const id of this.ids){
      ...
      if(current?.runId&&['running','user'].includes(current.state)){
        ...
        const receipt=await this.wait(`${this.receipts(id)}.find(r=>r.requestId===${JSON.stringify(requestId)})`,50000).catch(()=>null);

if(receipt?.status!=='applied')cleanup.errors.push(`Task cleanup not confirmed: ${id}`);
      }
    }

    if(this.pageSid){await this.p(`testPort.postMessage(${JSON.stringify({kind:'select_conversation',conversationId:this.original})})`).catch(()=>{});const restored=await this.wait(`chrome.storage.session.get('selectedConversationId').then(s=>s.selectedConversationId===${JSON.stringify(this.original)})`,5000).catch(()=>false);

if(!restored)cleanup.errors.push('Original conversation selection not restored');}

    for(const id of this.tabs){const removed=await this.w(`chrome.tabs.remove(${id}).then(()=>true).catch(()=>chrome.tabs.get(${id}).then(()=>false).catch(()=>true))`).catch(()=>false);

if(removed)cleanup.removedTabs.push(id);else cleanup.errors.push(`Tab cleanup not confirmed: ${id}`);}

    if(this.targetId){const closed=await this.connection.cdp.send('Target.closeTarget',{targetId:this.targetId}).catch(()=>null);cleanup.probeClosed=closed?.success===true;

if(!cleanup.probeClosed)cleanup.errors.push('Probe cleanup not confirmed');}else cleanup.probeClosed=true;
    await this.connection.close().catch(()=>cleanup.errors.push('CDP cleanup failed'));cleanup.ok=cleanup.errors.length===0;

if(!cleanup.ok){if(this.lastReport)this.lastReport.ok=false;process.exitCode=1;}
