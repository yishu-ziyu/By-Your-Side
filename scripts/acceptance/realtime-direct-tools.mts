...
const wait=async(fn:()=>any,label:string,ms=30000)=>{const end=Date.now()+ms;

while(Date.now()<end){const value=await fn();

if(value)return value;await new Promise(r=>setTimeout(r,150));}
...
return host.manager.routeVoiceInput(id,text,started,current,context);},
  ...
  async(request:any,current:any)=>{oldRoutes++;const receipt=await host.manager.dispatchTaskAction(request,current);

return {ok:['queued','accepted','applied'].includes(receipt.status),status:receipt.status,receipt};},
  async(id:string,call:any,input:any,signal:any)=>{directCalls.push({name:call.name,args:call.args});

return host.manager.executeRealtimeBrowserTool(id,call,input,signal);});
 ...
 host.wss.on('connection',(client:any)=>{
  const send=client.send.bind(client);client.send=(raw:any,...args:any[])=>{const m=JSON.parse(String(raw));
...
return send(raw,...args);};
 ...
 await wait(async()=>{const result=await iso.evalIn(target,judgeMode?`({value:document.documentElement.lang,saved:window.__saved===true})`:`({value:document.querySelector('#code').value,saved:window.__saved===true})`);

if(result.value===(judgeMode?'zh':'星河')){report.result=result;

return true;}},'field change',45000);
 ...
 voice?.close();await iso?.close().catch(()=>{});
