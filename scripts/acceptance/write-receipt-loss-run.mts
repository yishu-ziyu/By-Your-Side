/** Controller-owned frozen evaluator. See docs/evals/20260909-write-receipt-loss.md.
...
const out=await mkdtemp(join(tmpdir(),'ego-write-receipt-loss-'));
...
try{
 ...
 const gate=new ControlGate();const handlers={switch_tab:switchTab,snapshot,click};
...
return r.result?.value;};
 ...
 for(const fault of ['normal','timeout','disconnect']){
...
if(bridgeError)throw bridgeError;
  ...
  if(fault==='normal'){
   ...
   progress.registerResults([intent]); // A model may legally attempt this; host must preserve uncertainty.
   record.retry=await invoke('click',fault+'-retry',{target:'#append'});
   await Promise.all(bridgeJobs);
