...
try{
  ...
  const open=async url=>{const t=await evaluateInWorker(cdp,swSession,`chrome.tabs.create({url:${JSON.stringify(url)},active:true})`);await until(()=>evaluateInWorker(cdp,swSession,`chrome.tabs.get(${t.id}).then(t=>t.status==='complete')`),'page load');

return await evaluateInWorker(cdp,swSession,`chrome.tabs.get(${t.id})`);};
  ...
  await until(async()=>{const es=await events();

return toolEvents(es,sentA).some(e=>e.kind==='tool_end'&&!e.isError&&((e.name==='switch_tab'&&e.resultText?.includes(String(tabA.id)))||(e.name==='snapshot'&&e.resultText?.includes('WORK_PAGE_ALPHA'))));},'task A page observation',120000);
...
return toolEvents(es,result.handbackAt).some(e=>e.kind==='agent_end')&&stateOf(es)==='idle';},'task A completed after handback',120000);
...
return toolEvents(es,sentB).some(e=>e.kind==='agent_end')&&stateOf(es)==='idle';},'task B completed in same native session',150000);
  ...
  for(const name of (await readdir(traceDir)).filter(n=>n.endsWith('.jsonl'))){const path=join(traceDir,name),body=await readFile(path,'utf8');

if(body.includes(markerA)||body.includes(markerB)){const rows=body.trim().split('\n').map(s=>JSON.parse(s));matches.push({path,sessionIds:[...new Set(rows.map(r=>r.sessionId))],hasA:body.includes(markerA),hasB:body.includes(markerB)});}}
  ...
  if(cdp&&panel){await save('final-events.json',await evaluateInWorker(cdp,panel.session,'globalThis.__newTaskEvents||[]').catch(()=>[]));

if(ownsTask)await evaluateInWorker(cdp,panel.session,`globalThis.__newTaskPort?.postMessage({kind:'client',msg:{type:'abort'}})`).catch(()=>{});}
