...
if(suite==='extensions'){
 for(let repeat=1;repeat<=3;repeat++){console.log(`extensions repetition ${repeat}/3`);

for(const file of ['voice-control-run.mts','voice-context-run.mts','voice-target-run.mts'])await child(['--import','tsx',`scripts/acceptance/${file}`]);}
...
const out=`/tmp/ego-voice-dispatch-${Date.now()}`;
...
async function until(test:()=>boolean,ms=150000){const end=Date.now()+ms;

while(!test()){if(report.voiceFailure)throw Error(report.voiceFailure);

if(Date.now()>end)throw Error('Acceptance timeout');await sleep(100);}}
...
let turn=0,ready=false,done=false;
...
const check=(name:string,ok:boolean)=>{report.checks.push({name,ok});
...
try{
 ...
 manager=new ConversationManager((id,emit)=>createConversationRuntime(id,emit,'minimax-cn/MiniMax-M3'),(m:any)=>{
  ...
  if(m.type==='tool_call'){
   ...
   }).catch((e:unknown)=>{report.bridgeError=String(e);});

jobs.add(p);void p.finally(()=>jobs.delete(p));
  }
 });
 ...
 const opened=await connection.tool(cid,'main','open_tab',{url});

if(!opened.ok)throw Error(opened.error);tabs.add(opened.data.tabId);report.tabId=opened.data.tabId;
 voice=new StepVoiceSession({connect:key=>{connections++;

return upstream=new WebSocket(STEP_VOICE_ENDPOINT,{headers:{Authorization:`Bearer ${key}`},handshakeTimeout:12000,followRedirects:false});},getSnapshot:()=>manager!.getTaskProgress(cid),route:async(text,at,current,context)=>{const r=await manager!.routeVoiceInput(cid,text,at,current,context);

if(suite==='faults'&&!dropped&&'receipts' in r&&r.receipts?.some(r=>r.action==='start'&&r.status==='accepted')){dropped=true;upstream?.terminate();}

return r;},diagnostic:(event,fields)=>report.diagnostics.push({at:Date.now(),event,...fields}),emit:e=>{
...
async function speak(text:string){
 ...
 voice!.command({kind:'interrupt',turn:n});

for(let i=0;i<pcm.length;i+=960){voice!.command({kind:'audio',turn:n,data:pcm.subarray(i,i+960).toString('base64')});await sleep(20);}

 voice!.command({kind:'commit',turn:n});

if(suite==='faults')voice!.command({kind:'commit',turn:n});await until(()=>done,40000);
}
