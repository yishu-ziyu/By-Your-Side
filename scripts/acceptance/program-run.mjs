...
if(config.proxy){const proxy=new ProxyAgent(config.proxy),direct=new Agent();setGlobalDispatcher(new Agent({factory(origin){const host=typeof origin==='string'?new URL(origin).hostname:origin.hostname;

return ['localhost','127.0.0.1','::1','[::1]'].includes(host)?direct:proxy;}}));}
...
return result;
    ...
    const tool=async(name,params={})=>{const r=await raw(name,params);
...
return r.data;};
    ...
    try{
      ...
      agent=await BrowserAgentSession.create(rpc,{emit(e){events.push({at:Date.now(),...e});

if(e.kind==='agent_start')active=true;

if(e.kind==='agent_end')ended=true;

if(e.kind==='error')error=e.message;

if(e.kind==='text_delta')text.push(e.delta);

if(e.kind==='tool_start')console.log(`${task}/${mode}: ${e.name}`);},setStatus(){}},{modelPattern:'minimax-cn/MiniMax-M3',customTools:tools,systemPrompt:base});
      ...
      if(task==='extract'){const match=text.join('').match(/\[\s*\{[\s\S]*\}\s*\]/);

if(match)try{actual=JSON.parse(match[0]);}catch{}}
    ...
    finally{await writeFile(join(dir,'events.json'),JSON.stringify(events,null,2));await writeFile(join(dir,'result.json'),JSON.stringify(results.at(-1),null,2));agent?.dispose();

if(tabId)await raw('close_tab',{tabId}).catch(()=>{});console.log(JSON.stringify(results.at(-1)));}
  }
