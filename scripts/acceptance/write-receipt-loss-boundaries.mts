...
function setup(){
 ...
 const raw:any={subscribe:(f:any)=>{emit=f;
...
return()=>{};}};
 ...
 async function invoke(name:string,id:string,params:any){
  ...
  try{const result=await tools.find(t=>t.name===name)!.execute(id,params);emit({type:'tool_execution_end',toolCallId:id,toolName:name,isError:false,result});

return{ok:true};}
  catch(e){emit({type:'tool_execution_end',toolCallId:id,toolName:name,isError:true,result:{content:[{type:'text',text:String(e)}]}});
...
await writeFile('docs/evals/20260909-write-receipt-loss-independent-boundaries.json',JSON.stringify(report,null,2)+'\n');
