...
const out=resolve(`out/acceptance/20260920-general-browser/voice-dev-${Date.now()}`);
...
const events:any[]=[],diagnostics:any[]=[],dispatches:any[]=[],providerInputs:any[]=[];

let host:any,iso:any,voice:any,panel:string|undefined,oldRoutes=0;
...
const wait=async(fn:()=>any,label:string,ms=70000)=>{const end=Date.now()+ms;
...
if(host.socket?.readyState===WebSocket.OPEN)host.socket.send(JSON.stringify(msg));},undefined,(deps:any)=>new RealtimeVoiceSession({...deps,connect:(key:string)=>{
   ...
   ws.on('message',(raw:any)=>{const e=JSON.parse(String(raw));

if(e.type==='input_audio_buffer.speech_started')order.set(e.item_id,order.size);

if(e.type==='conversation.item.input_audio_transcription.completed')providerInputs.push({id:e.item_id,order:order.get(e.item_id),text:e.transcript});});

return ws;
  }}),undefined,
  (id:string,text:string,started:any,current:any,context:any)=>{oldRoutes++;
...
return host.manager.routeVoiceInput(id,text,started,current,context);},
  ...
  async(request:any,current:any)=>{dispatches.push({at:Date.now(),request});const receipt=await host.manager.dispatchTaskAction(request,current);
...
return {ok:['queued','accepted','applied'].includes(receipt.status),status:receipt.status,message:receipt.message,receipt};});
 ...
 host.wss.on('connection',(client:any)=>{const send=client.send.bind(client);client.send=(raw:any,...args:any[])=>{const m=JSON.parse(String(raw));

if(m.type!=='voice')voice.observe(m);
...
return send(raw,...args);};});
 ...
 if(switchTab){const other=await iso.newTarget(iso.fixtureOrigin+'/other');await iso.evalIn(other,`document.title='资料页'`);wantedTab=await iso.swEval(`chrome.tabs.query({}).then(t=>t.find(x=>x.url===${JSON.stringify(iso.fixtureOrigin+'/other')})?.id)`);

if(!wantedTab)throw Error('target tab setup failed');}
...
}catch(e){report.error=String(e);}finally{clearTimeout(timer);voice?.close();await iso?.close().catch(()=>{});

if(host)await stopHost(host).catch(()=>{});report.transcripts=events.filter(e=>e.message.event?.kind==='text').map(e=>e.message.event);await save();console.log(JSON.stringify({out,passed:report.passed,error:report.error,result:report.result,asr:report.asr,utteranceShape:report.utteranceShape,oldRoutes,dispatchCount:dispatches.length},null,2));
