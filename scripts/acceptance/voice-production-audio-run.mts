...
const meterName=`acceptance-onset-${Date.now()}.js`,meterFile=new URL(`../../extension/dist/${meterName}`,import.meta.url);

let meterCreated=false;

let h:NativeVoiceHarness|undefined;

const report:any={ok:false,kind:'synthetic-production-audio',rounds,emptyInterruption,latencyScope:'short arithmetic chat and idle task status',humanMicrophone:false,timings:[]};
...
try{
 ...
 for(let i=0;i<200;i++){const ready=await h.p(`audioLog.some(x=>x.kind==='phase'&&x.phase==='listening')`);

if(ready)break;

if(i===199)throw Error('Production client did not become ready');await sleep(100);}
 ...
 for(let round=0;round<rounds;round++){
  ...
  const pcm=await readFile(base+'.pcm');let last=pcm.length/2-1;

while(last>0&&Math.abs(pcm.readInt16LE(last*2))<492)last--;
...
for(let n=0;n<200;n++){heard=await h.p(`audioLog.some(x=>x.kind==='event'&&x.event.kind==='text'&&x.event.role==='user'&&x.event.turn===1)`);

if(heard)break;await sleep(50);}

if(!heard)throw Error('Original speech was not fully transcribed');
...
for(let i=0;i<650;i++){
 ...
 report.functionalPass=true;report.ok=emptyInterruption?true:report.latencyPass;

if(rounds===20&&!report.ok)process.exitCode=1;
}catch(error){report.error=String(error);process.exitCode=1;}
finally{if(h){await h.p('productionClient?.stop();captureContext?.close()').catch(()=>{});await h.finishReport(report);await h.close();console.log(JSON.stringify({out:h.out,ok:report.ok,error:report.error,p95Ms:report.p95Ms,latencyPass:report.latencyPass}));}
