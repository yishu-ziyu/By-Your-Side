...
try{result=JSON.parse(log.trim().split('\n').at(-1)!);}catch{result={ok:false,error:'missing final report'};}

results.push({name,exitCode,...result});await writeFile(join(suiteOut,'result.json'),JSON.stringify({suiteOut,ok:results.length===5&&results.every(r=>r.ok),results},null,2));console.log(JSON.stringify({name,...result}));
 }
...
const ext=join(out,'extension'),profile=join(out,'profile');
...
const server=createServer(async(req,res)=>{
 ...
 if(pathname==='/sidepanel.html'){res.setHeader('Content-Type','text/html; charset=utf-8');res.end((await readFile('extension/sidepanel.html','utf8')).replace('<script type="module"','<script src="mock.js"></script><script type="module"'));

return;}

 if(pathname==='/tone.wav'){res.setHeader('Content-Type','audio/wav');res.end(wav(Buffer.alloc(48000*180)));

return;}

 if(pathname==='/mock.js'){res.setHeader('Content-Type','text/javascript');res.end(uiMock);

return;}

 if(/\.(js|css|woff2|svg|png)$/.test(pathname)){try{const p=resolve('extension/dist','.'+pathname);

if(!p.startsWith(resolve('extension/dist')+'/'))throw Error('path');res.setHeader('Content-Type',({'.js':'text/javascript','.css':'text/css','.woff2':'font/woff2','.svg':'image/svg+xml'} as any)[extname(p)]??'application/octet-stream');res.end(await readFile(p));}catch{res.statusCode=404;res.end();}
...
return;
 ...
 if(layout==='live'){res.end((await readFile('docs/previews/s2-live-check.html','utf8')).replace('id="temporary-model"','id="target"').replace('id="standard-model"','id="other"'));
...
return;}
 ...
 if(caseName==='recovery'){
  ...
  res.end(`<!doctype html><meta charset="utf-8"><title>通知</title><style>body{font:22px sans-serif;padding:50px}article{max-width:700px;padding:20px;margin-bottom:50px;border:1px solid #ddd}p{margin:20px 0}</style><h1>本周消息</h1><article id="target"><h2>场地安排</h2><p>明天保持原计划。</p></article><article id="other">${target}</article>`);
...
let holdObservation=!['streaming_voice','live_dialogue','state_environment','state_tools'].includes(caseName);

const observationBarrier=new Promise<void>(r=>releaseObservation=r);
...
const bufferedPanel:any[]=[];

let panelEval:((text:string)=>Promise<any>)|undefined;
...
async function until<T>(fn:()=>Promise<T|undefined>|T|undefined,ms=30000):Promise<T>{const end=Date.now()+ms;

while(Date.now()<end){const value=await fn();

if(value)return value;await sleep(100);}

throw Error('Timeout waiting for evaluator stage: '+report.stage);}
...
try{
 ...
 const sid=await until(async()=>{const r=await cdp!.send('Target.getTargets');

for(const t of r.targetInfos.filter((t:any)=>t.type==='service_worker'&&t.url.startsWith('chrome-extension://'))){const sid=await cdp!.attachSession(t.targetId);const name=await cdp!.send('Runtime.evaluate',{expression:'chrome.runtime.getManifest().name',returnByValue:true},sid);

if(name.result?.value==='Isolated voice conversation evaluator')return sid;}

return undefined;});

 const evaluate=async(expression:string)=>{const r=await cdp!.send('Runtime.evaluate',{expression,awaitPromise:true,returnByValue:true},sid,45000);
...
return r.result?.value;};
 ...
 panelEval=async(expression:string)=>{const r=await cdp!.send('Runtime.evaluate',{expression,awaitPromise:true,returnByValue:true,userGesture:true},uiSid);
...
return r.result?.value;};
 ...
 const postPanel=(m:any)=>{if(!panelReady){bufferedPanel.push(m);

return;}

panelQueue=panelQueue.then(()=>panelEval!(`uiEmit(${JSON.stringify({kind:'server',msg:m})})`)).catch(error=>{report.transportError=String(error);});};

 cdp.onEvent('Runtime.bindingCalled',(event:any)=>{if(event.sessionId!==uiSid||event.params.name!=='hostSend')return;const envelope=JSON.parse(event.params.payload);

if(envelope.kind!=='client')return;pendingInput=pendingInput.then(async()=>{const m=envelope.msg;

if(m.type==='voice'&&['commit','playback_done'].includes(m.command.kind))(report.clientTiming??=[]).push({voiceId:m.voiceId,command:m.command.kind,turn:m.command.turn,responseId:m.command.responseId,at:Date.now()});

if(m.type==='voice'){voiceId=m.voiceId;

if(m.command.kind==='interrupt')turn=m.command.turn;

if(m.command.kind==='commit'){const observation=await evaluate('probe.issue()');

if(observation){const tab=await evaluate(`chrome.tabs.get(${observation.tabId})`);m.command={...m.command,input:{...m.command.input,context:{tabId:tab.id,url:tab.url,title:tab.title},observation}};}}

await voice!.handle(m.conversationId??'default',m);}else await manager!.handleMessage(m);}).catch(error=>{report.transportError=String(error);});});
 const store=new ConversationStore(join(out,'conversations'));
 manager=new ConversationManager((id,emit,summary)=>createConversationRuntime(id,emit,'minimax-cn/MiniMax-M3',{sessionManager:store.sessionManager(id),mode:summary?.mode,customTools:caseName==='live_dialogue'?[defineTool({name:'wait_for_fixture',label:'等待测试资料',description:'Wait until the local test fixture is ready, then read the current page with browser tools.',parameters:Type.Object({}),execute:async()=>{observationHeld=true;report.observationBarrierAt=Date.now();await observationBarrier;

return {content:[{type:'text',text:'测试资料已就绪，请读取当前页面。'}],details:{ready:true}};}})]:caseName.startsWith('extra_tool')?[defineTool({name:'fixture_meter',label:'测试仪表',description:'Read the current reading of the test instrument. The number is available only from this tool, not the page or prior conversation.',parameters:Type.Object({}),execute:async()=>({content:[{type:'text',text:String(meterValue)}],details:{reading:meterValue}})})]:[]}),message=>{
  messages.push(message);voice?.observe(message);postPanel(message);

if(message.type==='agent_event'&&['tool_end','agent_end','user_delivery','user_delivery_stream'].includes(message.event.kind))(report.progressEvents??=[]).push({event:message.event,at:Date.now(),snapshot:manager?.getTaskProgress('default')});
  ...
  if(message.type==='tool_call'){
   ...
   void evaluate(`probe.execute(${JSON.stringify(message)})`).then(async data=>{record.ok=true;record.result=JSON.stringify(data,(key,value)=>key==='imageBase64'?'[image omitted]':value)?.slice(0,16000);

if(message.name==='mark'){record.annotationBoxes=await marks();record.pageState=await pageState();}

if(holdObservation&&['snapshot','read_element'].includes(message.name)){holdObservation=false;observationHeld=true;report.observationBarrierAt=Date.now();await observationBarrier;}

return manager!.handleMessage({type:'tool_result',conversationId:message.conversationId,id:message.id,ok:true,data});},async error=>{record.ok=false;record.error=String(error);

if(caseName==='takeover'&&message.name==='mark'){await manager!.handleMessage({type:'takeover',conversationId:message.conversationId,requestId:'injected-takeover',members:[{sessionId:'main',role:'lead',tabId:fixtureTab.id,activity:'running'}]});}

return manager!.handleMessage({type:'tool_result',conversationId:message.conversationId,id:message.id,ok:false,error:String(error)});});
  }
 },store);
 voice=new VoiceService(id=>manager!.getTaskProgress(id),message=>{
  messages.push(message);postPanel(message);

if(message.type!=='voice')return;
  const e=message.event;

if(e.kind==='audio'){if(!audio.has(e.responseId))(report.audioTiming??=[]).push({voiceId:message.voiceId,turn:e.turn,responseId:e.responseId,firstAudioAt:Date.now()});const chunks=audio.get(e.responseId)??[];chunks.push(Buffer.from(e.data,'base64'));audio.set(e.responseId,chunks);}else report.voice.push({voiceId:message.voiceId,event:e,at:Date.now()});
...
for(const m of bufferedPanel.splice(0))postPanel(m);await panelQueue;
 ...
 const speak=async(text:string,_input?:VoiceInputContext)=>{
  ...
  await until(()=>{if(report.transportError)throw Error(report.transportError);const events=messages.slice(begin);

return events.find(m=>m.type==='agent_event'&&m.event.kind==='user_delivery'&&m.event.delivery?.kind!=='ack');},120000);
 ...
 const marks=async()=>{
  ...
  const walk=(n:any)=>{const attrs=n.attributes??[],index=attrs.indexOf('class');

if(index>=0&&/(^| )mark( |$)/.test(attrs[index+1]))nodes.push(n);

for(const child of [...n.children??[],...n.shadowRoots??[]])walk(child);};

walk(tree.root);
  const boxes:any[]=[];

for(const node of nodes){const r=await evaluate(`chrome.debugger.sendCommand({tabId:${fixtureTab.id}},'DOM.getBoxModel',{backendNodeId:${node.backendNodeId}}).catch(()=>null)`);

if(r?.model){const q=r.model.border;boxes.push({x:Math.min(q[0],q[2],q[4],q[6]),y:Math.min(q[1],q[3],q[5],q[7]),width:Math.max(q[0],q[2],q[4],q[6])-Math.min(q[0],q[2],q[4],q[6]),height:Math.max(q[1],q[3],q[5],q[7])-Math.min(q[1],q[3],q[5],q[7])});}}

return boxes;
 };

 const sendText=async(text:string)=>{const from=messages.length;await panelEval!(`(()=>{document.querySelector('#input').value=${JSON.stringify(text)};document.querySelector('#input').dispatchEvent(new Event('input',{bubbles:true}));})()`);await panelEval!(`document.querySelector('#input').focus()`);await cdp!.send('Input.dispatchKeyEvent',{type:'keyDown',key:'Enter',code:'Enter',windowsVirtualKeyCode:13},uiSid);await cdp!.send('Input.dispatchKeyEvent',{type:'keyUp',key:'Enter',code:'Enter',windowsVirtualKeyCode:13},uiSid);await evaluate(`chrome.tabs.update(${fixtureTab.id},{active:true})`);
...
try{await execute('read_element',{target:'@'+ref,expect:{property:'paused',equals:true}});}catch(error){failed=String(error).includes('条件未满足');}
...
try{await execute('read_element',{target:'article',properties:['visible']});}catch(error){failed=String(error).includes('匹配 3');}
...
try{await execute('read_element',{target:'#delayed',expect:{property:'checked',equals:true},timeoutMs:1500});}catch(error){failed=String(error).includes('文档已变化');}
  ...
  await until(async()=>{const s=await media();

return !s.paused&&s.time>0?s:undefined;},10000);
...
try{await execute('mark',{target:'@'+headings[1]});}catch{staleRejected=true;}
 ...
 if(caseName==='recovery'){
  ...
  report.stage='correction accepted';const correctionFrom=messages.length;const speech=caseName==='correction_voice'?speak('对象改成标准模型ID，其他要求保留。'):null;

if(!speech)await sendText(layout==='live'?'改成标准模型，其他要求不变。':'对象改成标准模型ID，其他要求保留。');
  ...
  report.correctionAcceptedAt=Date.now();check('correction preserves original run',manager.getTaskProgress('default')?.runId===origin.runId);releaseObservation();

if(speech)report.speechInputResult=await speech;await settle(start);await onlyOther();
...
try{await evaluate(`probe.execute({name:'mark',params:{target:'#target'},conversationId:'default'})`);}catch{blocked=true;}

check('production control gate rejects paused write',blocked);await noMark();
 ...
 if(['correction','correction_voice','pause'].includes(caseName)){
  ...
  const beforeReplay=report.tools.length;const prior=await panelEval!(`uiMessages.find(m=>m.kind==='client'&&m.msg.type==='task_action'&&m.msg.request.action==='start')?.msg`);

if(prior)await manager.handleMessage(prior);
  const delivery=report.finalSnapshot.conversationContext?.latestDelivery;

if(delivery){manager.markDeliveryPlayback('default',delivery.id,'played');manager.markDeliveryPlayback('default',delivery.id,'played');}
  ...
  const done=report.finalSnapshot.results.find((r:any)=>r.tool==='mark'&&r.status==='satisfied');let duplicateBlocked=false;

try{await (manager.get('default')!.runtime.session as any).session.getToolDefinition('mark').execute('duplicate-probe',{target:done.target});}catch{duplicateBlocked=true;}
...
if(child){const exited=new Promise<void>(r=>child!.once('exit',()=>r()));child.kill('SIGTERM');await Promise.race([exited,sleep(3000)]);report.browserExited=child.exitCode!==null||child.signalCode!==null;

if(!report.browserExited){child.kill('SIGKILL');await exited;report.browserExited=true;}}
