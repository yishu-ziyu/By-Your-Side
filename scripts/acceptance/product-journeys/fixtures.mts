...
const CANARY = `<script>(()=>{const snap=()=>((document.querySelector('main')||document.body).innerText||'').replace(/\\s+/g,' ').trim();const start=()=>{const f=window.__fixture={contentMutations:0,attrMutations:0,initialText:snap()};new MutationObserver(list=>{for(const m of list){if(m.type==='attributes')f.attrMutations++;else f.contentMutations++;}}).observe(document.querySelector('main')||document.body,{subtree:true,childList:true,characterData:true,attributes:true});};if(document.readyState==='loading')addEventListener('DOMContentLoaded',start);else start();})();</script>`;
