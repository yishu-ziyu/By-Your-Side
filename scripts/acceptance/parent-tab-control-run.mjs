...
async function main() {
  ...
  const check=(name,ok)=>{evidence.checks.push({name,ok});

if(!ok)throw Error(name);console.log('PASS '+name);};

  async function call(name,params={},who=worker,conv=cid) { const r=await tool(conv,who,name,params);evidence.calls.push({name,who,conv,...r});

return r; }

  try {
    for(let n=0;n<2;n++){const r=await call('open_tab',{url});

if(!r.ok)throw Error(r.error);opened.push(r.data.tabId);}
    ...
    for(let n=0;n<50;n++){const r=await call('read_element',{tabId:opened[1],target:'#result'});

if(JSON.stringify(r).includes('正在完成旧操作'))break;

if(n===49)throw Error('old operation never started');await new Promise(r=>setTimeout(r,30));}
  ...
  finally {await writeFile(output+'/result.json',JSON.stringify(evidence,null,2));

for(const id of opened)await evaluateInWorker(cdp,sid,`chrome.tabs.remove(${id}).catch(()=>{})`).catch(()=>{});await connection.close();}
}
