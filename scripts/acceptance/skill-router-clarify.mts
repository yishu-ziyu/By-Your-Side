...
const out=resolve('out/acceptance',`skill-router-clarify-${Date.now()}`);await mkdir(out,{recursive:true});
...
try{
  ...
  for(const [index,f] of fixtures.entries()){
    ...
    const result:SkillRoute=await routeSkill({userText:f.text,hostname:'example.com',skills:[skill]},
      {judge:async(input,signal)=>{liveCalls++;judgment=await judgeSkill(input,signal);return judgment as never;}});
