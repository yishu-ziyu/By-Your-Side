...
if(!validOracle(oracle))throw Error('Invalid independent oracle');
...
if(!reader)throw Error('Empty oracle response');
   const chunks:Uint8Array[]=[];let size=0;

   try{for(;;){const part=await reader.read();

if(part.done)break;size+=part.value.length;

if(size>65536)throw Error('Oracle response too large');chunks.push(part.value);}}finally{await reader.cancel().catch(()=>{});}
