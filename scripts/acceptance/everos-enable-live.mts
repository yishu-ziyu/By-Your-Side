...
const out=resolve('docs/evals/20260916-everos-enable/live');

await mkdir(out,{recursive:true});

const token=randomUUID(),model=loadConfig().model;

const runtimeDir=resolve('out/acceptance',`everos-enable-live-${Date.now()}`);
...
return tabs.find(t=>t.url===iso!.fixtureOrigin+'/article')},5000,'fixture tab');
 ...
 const record=await until(async()=>{const records=await dailyExperiences.list(cid);

return records.find(r=>r.goal===goal&&r.startedAt>=started&&r.endedAt)},10000,'persisted production experience');
 ...
 await writeFile(join(out,'events.json'),JSON.stringify(events,null,2));await iso?.close();manager.dispose();
