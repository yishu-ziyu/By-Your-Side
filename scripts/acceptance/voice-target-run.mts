...
try{
 ...
 h=await NativeVoiceHarness.open('voice-target');const existing=await h.w(`(globalThis.__saServerEvents||[]).filter(e=>e.type==='conversation_list').at(-1)?.conversations.map(c=>c.title)||[]`);const name=['春日旅行','夏日旅行','秋日旅行','冬日旅行','海边旅行','山间旅行','森林旅行','草原旅行','湖边旅行','沙漠旅行','雪山旅行','乡村旅行','清晨旅行','晚风旅行','星空旅行','雨后旅行','远山旅行','小岛旅行','溪谷旅行','落日旅行','白云旅行','晴空旅行','北方旅行','南方旅行'].find(n=>!existing.includes(n));

if(!name)throw Error('No unused fixture name');const a=await h.create('自动验收 · 来源'),b=await h.create(name);const tab=await h.openTab(b,url);await h.listen(a);
...
for(let i=0;i<100;i++){restored=await h.p(planRows).then(n=>n===2).catch(()=>false);

if(restored)break;await new Promise(r=>setTimeout(r,100));}
