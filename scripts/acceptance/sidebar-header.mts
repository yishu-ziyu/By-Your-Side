...
const out=resolve(outArg?outArg.slice(6):'docs/evals/20260916-sidebar-header');
...
const server=createServer(async(req,res)=>{try{
 ...
 if(path==='/mock.js'){res.setHeader('Content-Type','text/javascript');res.end(mock);
...
return;}
 ...
 if(path==='/'||path==='/sidepanel.html'){res.setHeader('Content-Type','text/html');res.end((await readFile('extension/sidepanel.html','utf8')).replace('<script type="module"','<script src="mock.js"></script><script type="module"'));

return;}

 const p=resolve('extension/dist','.'+path);

if(!p.startsWith(resolve('extension/dist')+'/'))throw Error('path');
...
async function until<T>(fn:()=>Promise<T|undefined>,ms=15000){const end=Date.now()+ms;

while(Date.now()<end){const v=await fn();

if(v)return v;await sleep(100);}

throw Error('UI evaluator timeout: '+report.stage);}
...
try{
 ...
 const evaluate=async(expression:string)=>{const r=await cdp!.send('Runtime.evaluate',{expression,awaitPromise:true,returnByValue:true,userGesture:true},sid);
...
return r.result?.value;};
 ...
 const check=async(name:string,expression:string)=>{const ok=!!await evaluate(expression);report.clicks.push({name,ok});
...
if(child){const exited=new Promise<void>(r=>child!.once('exit',()=>r()));child.kill('SIGTERM');await Promise.race([exited,sleep(3000)]);
