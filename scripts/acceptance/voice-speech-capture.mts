...
const v=await fetch('http://127.0.0.1:9222/json/version').then(r=>r.json()) as any;

const c=createCdp(v.webSocketDebuggerUrl);

await c.ready();

try{const {targetInfos}=await c.send('Target.getTargets');const target=targetInfos.find((t:any)=>process.argv[3] ? t.targetId===process.argv[3] : t.url.startsWith('chrome-extension://fnbjglhppbkgmjeehablkfilmmefjolo/voice-permission.html?speech-check='));

if(!target)throw Error('Native test page not available');const sid=await c.attachSession(target.targetId);const r=await c.send('Runtime.evaluate',{expression:'(async()=>{'+result.outputFiles[0]!.text+';return globalThis.captureResult})()',awaitPromise:true,returnByValue:true,userGesture:true},sid,30000);

if(r.exceptionDetails)throw Error(r.exceptionDetails.exception?.description);console.log(JSON.stringify(r.result.value));await writeFile(process.argv[2]+'.capture.json',JSON.stringify(r.result.value,null,2));

if(!r.result.value?.ok)process.exitCode=1;}finally{await c.close()}
