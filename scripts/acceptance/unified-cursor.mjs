...
if(process.argv.includes('--motion')) {
  await check('list opens from top right with transition',()=>{const l=window.roots[0].querySelector('.xlist');return getComputedStyle(l).transitionDuration.includes('0.25s')&&!l.inert});
  await page.evaluate(()=>{const b=window.roots[0].querySelector('.xpage > button');b.click();b.click();b.click()});
  await check('rapid close is immediately inert',()=>{const l=window.roots[0].querySelector('.xlist');return l.hidden&&l.inert});
...
await check('only approval shows local buttons',()=>{const c=window.roots[0].querySelector('.cursor.holding');return !!c&&getComputedStyle(c.querySelector('.label')).display==='flex'&&c.querySelectorAll('button').length===2});
