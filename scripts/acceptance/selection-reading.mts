...
if(!msg)return;
  ...
  if(msg.type==='hello'){
    if(msg.token!==token){client.close();
...
socket=client;
    ...
    reply({type:'conversation_list',conversations:manager?.list()??mockSummaries});
...
return;
  ...
  if(manager){void manager.handleMessage(msg).catch(error=>{events.push({error:String(error)});});
...
return;}
  ...
  if(msg.type==='reading_request'){
    ...
    if(mode==='hold') {reply({...base,state:'streaming',text:'已生成的部分'});
...
return;}
  ...
  if(msg.type==='conversation_create'){const conversation={...summary,id:'c-'+msg.requestId.slice(0,8),title:msg.title??'新会话'};

if(!mockSummaries.some(s=>s.id===conversation.id))mockSummaries.push(conversation);reply({type:'conversation_created',requestId:msg.requestId,conversationId:conversation.id,conversation});}
}));
...
try {
  ...
  const find=(node:any):any=>{if(node.attributes?.includes('data-sideagent-ask'))return node;

for(const child of [...node.children??[],...node.shadowRoots??[]]){const found=find(child);
...
return result.result.value;
  ...
  async function click(selector:string){const point=await ui(`const el=this.querySelector(${JSON.stringify(selector)});if(!el||el.disabled)throw Error('control unavailable');const r=el.getBoundingClientRect();return {x:r.x+r.width/2,y:r.y+r.height/2};`);

for(const type of ['mousePressed','mouseReleased'])await cdp!.send('Input.dispatchMouseEvent',{type,button:'left',clickCount:1,...point},session);await sleep(80);}
  ...
  async function done(turns=1){await until(async()=>{const r=await latest();

return r?.turns.length===turns&&r?.turns.at(-1)?.state==='done'?r:undefined;},live?90000:10000,'reading complete');await sleep(250);}
  ...
  await until(async()=>{const text=await iso!.evalIn(panel,'document.body.innerText');

return text.includes('文中的实验内部代号')?text:undefined;},15000,'panel imported visible conversation');
...
finally {for(const entry of manager?.list()??[])manager?.get(entry.id)?.runtime.dispose();socket?.close();

for(const client of wss.clients)client.terminate();await new Promise<void>(r=>wss.close(()=>r()));await cdp?.close();await iso?.close();}
