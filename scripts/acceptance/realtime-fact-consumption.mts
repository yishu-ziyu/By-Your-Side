...
export async function runFactConsumption() {
 ...
 if(scenario) {
  ...
  await runCase(scenario,attempt,resolve(arg('facts-out')!));return;
 }
 ...
 try {
  ...
  console.log(`BLOCKED ${String(error)}`);process.exitCode=1;return;
 }
 for(const run of schedule) {
  ...
  await new Promise<void>((resolveRun)=>{
   ...
   child.once('close',(code,signal)=>{
    clearTimeout(timer);if(force)clearTimeout(force);killGroup('SIGKILL'); // Only this test's process group, including any orphaned Chrome child.
...
async function runCase(scenario:FactScenario,attempt:number,out:string) {
 ...
 const wait=async(fn:()=>any,label:string,ms:number)=>{
  const end=Date.now()+ms;while(Date.now()<end){check();const value=await fn();if(value)return value;await new Promise(r=>setTimeout(r,100));}throw Error(`${label} timeout`);
 };
 try {
  ...
  hostEvents.push=(...rows:any[])=>{for(const row of rows){const {at,...data}=row;record('host',data);}return Array.prototype.push.apply(hostEvents,rows);};
  ...
  voice=new VoiceService((id:string)=>host.manager.getTaskProgress(id),(message:any)=>{
   record('voice',message);if(host.socket?.readyState===WebSocket.OPEN)host.socket.send(JSON.stringify(message));
   ...
   socket.send=((raw:any,...args:any[])=>{const result=(send as any)(raw,...args);try{record('provider-out',JSON.parse(String(raw)));}catch{}return result;}) as typeof socket.send;
   return socket;
  }}),undefined,
  (id:string,text:string,started:any,current:any,context:any)=>{record('legacy-route',{text});return host.manager.routeVoiceInput(id,text,started,current,context);},
  (event:string,fields:any)=>record('diagnostic',{event,...fields}),()=>host.manager.voiceTargets(),undefined,undefined,undefined,
  async(id:string,input:any)=>{const result=await readVoicePage(host.manager.get(id).runtime.rpc,input);record('read-page',{result});return result;},
  async(request:any,current:any)=>{record('legacy-dispatch',{request});const receipt=await host.manager.dispatchTaskAction(request,current);return {ok:['queued','accepted','applied'].includes(receipt.status),status:receipt.status,receipt};},
  async(id:string,call:any,input:any,signal:any)=>{
   record('direct-start',{...call,conversationId:id});
   try{const result=await host.manager.executeRealtimeBrowserTool(id,call,input,signal);record('direct-end',{name:call.name,callId:call.callId,result});return result;}
  ...
  host.manager.handleMessage=(message:any)=>{
   if(message.type==='tool_result'){
    ...
    if(scenario==='C'&&!dropped&&pending.get(message.id)==='fill'&&message.ok&&message.executionFact==='executed'){
     dropped=true;record('injection',{kind:'drop-write-result',transportId:message.id,point:'extension→host before manager.handleMessage; original RPC 30s timeout unchanged'});return Promise.resolve();
    }
   }
   ...
   client.send=(raw:any,...args:any[])=>{
    ...
    if(message.type==='tool_call'){
     ...
     if(scenario==='D'&&!readRejected&&isPageRead(message.name)){
      ...
      queueMicrotask(()=>host.manager.handleMessage({type:'tool_result',id:message.id,ok:false,error:'TEST_INJECTED_READ_FAILURE: first page read transport unavailable',executionFact:'not_executed'}));return;
     }
  ...
  await wait(()=>{const reply=finalReply(events);return reply&&Date.now()-reply.doneAt>=1200?reply:null;},'post-tool final response',90000);
 } catch(error) {
  ...
  try{if(host){const {stopHost}=await import('./product-journeys/runner.mjs');await stopHost(host);}cleanup.hostClosed=true;}catch(error){cleanup.hostError=String(error);}
