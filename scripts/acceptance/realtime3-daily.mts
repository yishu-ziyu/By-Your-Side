...
const out=resolve('out/acceptance/20260920-realtime3-daily');
...
const wait=async(fn:()=>Promise<any>|any,label:string,ms=45000)=>{const end=Date.now()+ms;
...
if(host.socket?.readyState===WebSocket.OPEN)host.socket.send(JSON.stringify(msg));},undefined,undefined,undefined,
   ...
   async(id:string,input:any)=>{const p:any=await host.manager.get(id).runtime.rpc.call('observe_page',{token:input.observation?.token},8000);

return {text:p.text,title:p.title,url:p.url,scope:p.scope};});
 ...
 host.wss.on('connection',(client:any)=>{const send=client.send.bind(client);client.send=(raw:any,...args:any[])=>{const message=JSON.parse(String(raw));
...
return send(raw,...args);};});
 ...
 for(const f of frames){const b=Buffer.from(f.message.command.data,'base64');let p=0;

for(let n=0;n+1<b.length;n+=2)p=Math.max(p,Math.abs(b.readInt16LE(n)));peak=Math.max(peak,p);

if(p>0)nonzeroFrames++;}
...
if(host)await stopHost(host).catch(()=>{});
 report.diagnostics=diagnostics;report.voiceEvents=events.filter(e=>e.message.type==='voice').map(e=>{const m=e.message;

return {direction:e.direction,kind:m.event?.kind??m.command?.kind,turn:m.event?.turn??m.command?.turn,...(m.event?.kind==='state'?{state:m.event.state,detail:m.event.detail}:{}),...(m.event?.kind==='text'?{role:m.event.role,text:m.event.text}:{})};});await save();console.log(JSON.stringify({passed:report.passed,error:report.error,checks:report.checks,capture:report.capture}));
