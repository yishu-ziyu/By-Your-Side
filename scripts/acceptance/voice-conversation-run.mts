...
if(e.kind==='audio'){const chunks=audio.get(e.responseId)??[];chunks.push(Buffer.from(e.data,'base64'));audio.set(e.responseId,chunks);}else report.voice.push({voiceId:message.voiceId,event:e,at:Date.now()});
 ...
 const speak=async(text:string,input?:VoiceInputContext)=>{
  ...
  await until(()=>{const events=messages.slice(begin).filter(m=>m.type==='voice'&&m.voiceId===voiceId);const err=events.find(m=>m.event.kind==='state'&&m.event.state==='error');

if(err)throw Error(err.event.detail);

return events.find(m=>m.event.kind==='response_end'&&m.event.turn===t);},60000);
