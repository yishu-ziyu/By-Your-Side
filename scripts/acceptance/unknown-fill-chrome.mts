...
function hashes(root:string):Record<string,string> {
 ...
 function visit(dir:string){for(const name of readdirSync(dir,{withFileTypes:true})){const path=join(dir,name.name);if(name.isDirectory())visit(path);else if(name.isFile())result[relative(root,path)]=sha(readFileSync(path));}}
...
export function createDeliveryQueue(deliver:(message:any)=>Promise<void>,record:(kind:string,data:any)=>void) {
 ...
 return {
  push(message:any) {
   if(!accepting){record('delivery-rejected',{type:message.type,id:message.id});return false;}
   pending.add(message);
   tail=tail.then(async()=>{
    if(terminated){record('delivery-cancelled',{type:message.type,id:message.id});return;}
  ...
  async drain(ms=5000){await within(tail,ms,'delivery drain');if(errors.length)throw Error(errors.join('\n'));},
...
async function runCase(name:string) {
 ...
 globalThis.fetch=async(input,init)=>{
  ...
  record('network-allow',{channel:'fetch',url:String(url)});return originalFetch(input,init);
 };
 ...
 try {
  ...
  delivery=createDeliveryQueue(async(message:any)=>{
    if(message.type!=='tool_call'){await iso.swEval(`globalThis.__saHandleServer(${JSON.stringify(message)})`);return;}
    ...
    if(message.name==='fill'&&reply.ok&&reply.executionFact==='executed'&&!drop){
     ...
     const armed:any=await page(`window.__arm(${name==='B'})`);record('fixture-armed',armed);return;
    }
  ...
  const call=async(tool:string,args:Record<string,unknown>)=>{
   ...
   try{const result=await manager.executeRealtimeBrowserTool('default',request,input,abort.signal);record('test-result',{callId:request.callId,result});return result;}
  ...
  let writeBlocked=false;try{runtime.session.assertTaskResultExecution('fill',{tabId,target:fieldTarget,value:'星河'},'test-no-dispatch');}catch{writeBlocked=true;}
