...
async function fixtureRead() {
  ...
  for(const tab of tabs){const session=(await cdp.send('Target.attachToTarget',{targetId:tab.targetId,flatten:true})).sessionId;
    ...
    try {const r=await cdp.send('Runtime.evaluate',{expression:'({exports:globalThis.exportEvidence, text:document.body.innerText})',returnByValue:true},session);

if(r.result.value?.exports)views.push({...r.result.value,targetId:tab.targetId});}
...
async function changeFixture(total, layout='old') {
  ...
  for(let n=0;n<50;n++){try{const v=await fixtureRead();

if(v.text.includes('共 '+total+' 条'))return;}catch{}
