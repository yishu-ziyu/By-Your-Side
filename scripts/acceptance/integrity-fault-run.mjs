...
return r.data;};
  ...
  while(Date.now()<loadedUntil){const ts=await evaluateInWorker(cdp,session,`Promise.all([chrome.tabs.get(${alpha}),chrome.tabs.get(${beta})])`);

if(ts.every(t=>t.status==='complete'&&t.url.startsWith(server.origin))){loaded=true;break;}
...
await new Promise(r=>setTimeout(r,20));}
  ...
  const check=async(name,fn)=>{try{const r=await fn();results.push({name,...r});}catch(e){results.push({name,passed:false,error:String(e)});}
...
console.log(JSON.stringify(results.at(-1)));};
  ...
  const wait=Date.now()+10000;

while(Date.now()<wait){const t=await evaluateInWorker(cdp,session,`chrome.tabs.get(${alpha})`);

if(t.status==='complete'&&t.url===server.origin+'/observation-integrity.html')break;await new Promise(r=>setTimeout(r,20));}
...
if(cdp)await cdp.close();await server.close();console.log(JSON.stringify(report));process.exitCode=report.passed?0:1;
}
