...
class Socket extends EventTarget {
  ...
  addEventListener(type:string,fn:any,options?:any){super.addEventListener(type,fn,options);
...
for(const disconnect of [false,true])test(`CDP ${disconnect?'disconnect':'close'} settles requests and waits`,async t=>{
  ...
  t.mock.method(globalThis,'setTimeout',(fn:any)=>{const timer={fn};timers.add(timer);
...
return [{documentId:'probe-doc',result:{ready:'complete',url}}];}}},setTimeout,clearTimeout};
  ...
  class HookSocket extends Socket {
    send(raw:string){
      ...
      if(m.method==='Runtime.evaluate'){
        ...
        if(expression==='typeof globalThis.__saCall'){reply({result:{value:hooked?'function':'undefined'}});

return;}

        if(expression.includes('const tab = await chrome.tabs.create')){reply({result:{value:{tabId:3,documentId:'probe-doc'}}});
...
return;}
        ...
        if(expression.includes('const message = chrome.runtime.sendMessage')){
          ...
          if(mode==='trigger-failure'){reply({exceptionDetails:{text:'injection failed'}});

return;}

          if(mode==='disconnect'){queueMicrotask(()=>this.close());
...
return; // Trigger result deliberately held until SW resumes.
        ...
        reply({result:{value:[{result:{sent:true}}]}});

return;
      }

      if(m.method==='Debugger.enable'){
        reply();setTimeout(()=>this.message({method:'Debugger.scriptParsed',sessionId:'worker',params:{scriptId:'script',url:'chrome-extension://test/background.js'}}),200);
...
return;}
      ...
      if(m.method==='Debugger.setBreakpoint'){reply({breakpointId:'bp'});

return;}

      if(m.method==='Debugger.evaluateOnCallFrame'){
        assert.equal(m.params.callFrameId,'right');hooked=true;reply({result:{value:{ok:true}}});

return;
      }

      if(m.method==='Debugger.resume'){
        resumed=true;reply();queueMicrotask(()=>this.message({id:trigger.id,result:{result:{value:[{result:{sent:true}}]}}}));
...
test('delivery stops admission, drains in order, and terminates queued work before resource close',async()=>{
  ...
  const queue=createDeliveryQueue(async m=>{seen.push(m.id);

if(m.id===1)await new Promise<void>(r=>{release=r;});},(kind,data)=>events.push({kind,data}));
