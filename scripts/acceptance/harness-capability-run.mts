...
await voice!.handle(m.conversationId??'default',m);}else await manager!.handleMessage(m);}).catch(error=>{report.transportError=String(error);});});
 ...
 manager=new ConversationManager((id,emit,summary)=>createConversationRuntime(id,emit,'minimax-cn/MiniMax-M3',{sessionManager:store.sessionManager(id),mode:summary?.mode,customTools:caseName.startsWith('extra_tool')?[defineTool({name:'fixture_meter',label:'测试仪表',description:'Read the current reading of the test instrument. The number is available only from this tool, not the page or prior conversation.',parameters:Type.Object({}),execute:async()=>({content:[{type:'text',text:String(meterValue)}],details:{reading:meterValue}})})]:[]}),message=>{
  ...
  if(message.type==='tool_call'){
   ...
   void evaluate(`probe.execute(${JSON.stringify(message)})`).then(data=>{record.ok=true;record.result=JSON.stringify(data,(key,value)=>key==='imageBase64'?'[image omitted]':value)?.slice(0,16000);
...
return boxes;
 ...
 const sendText=async(text:string)=>{const from=messages.length;await panelEval!(`(()=>{document.querySelector('#input').value=${JSON.stringify(text)};document.querySelector('#input').dispatchEvent(new Event('input',{bubbles:true}));})()`);await click('#send-btn');await evaluate(`chrome.tabs.update(${fixtureTab.id},{active:true})`);

if(caseName==='takeover'){await until(()=>messages.slice(from).find(m=>m.type==='control_result'&&m.action==='takeover'&&m.ok),120000);}else{await until(()=>messages.slice(from).find(m=>m.type==='agent_event'&&m.event.kind==='user_delivery'&&m.event.delivery.kind!=='ack'),120000);await until(()=>messages.slice(from).find(m=>m.type==='agent_event'&&m.event.kind==='agent_end'),120000);}
...
return from;};
 ...
 const utterance=caseName==='heldout'?'帮我在这页给临时模型ID做个醒目的圈注。':caseName==='root_target'?'root guard':caseName==='js_enabled'?'JS execution regression':utterances[caseName];

if(!utterance)throw Error('Unknown case '+caseName);report.utterance=utterance;
 const start=messages.length;

 if(caseName==='root_target'){await evaluate(`probe.execute({name:'switch_tab',params:{tabId:${fixtureTab.id}},conversationId:'default',sessionId:'main'})`);let rejected=false;

try{await evaluate(`probe.execute({name:'mark',params:{target:'body'},conversationId:'default',sessionId:'main'})`);}catch(error){rejected=String(error).includes('具体内容');report.rootError=String(error);}

check('the actual browser mark tool rejects a whole-page placeholder',rejected);}
 ...
 if(wantsMark){
  ...
  const findSvg=(n:any)=>{const attrs=n.attributes??[],i=attrs.indexOf('class');

if(i>=0&&/(^| )sketch-svg( |$)/.test(attrs[i+1]))svgId=n.backendNodeId;

for(const ch of [...n.children??[],...n.shadowRoots??[]])findSvg(ch);};
