...
export async function main():Promise<number> {
  ...
  try {stepKey=await readStepVoiceKey();

if(!readTypeSafeKey())throw new Error('missing_jev_credential');}
  catch {await writeFile(summaryPath,JSON.stringify({status:'BLOCKED',reason:'missing_credentials',scenarios:SCENARIOS.map(s=>({id:s.id,status:'NOT_RUN'}))},null,2));console.log(summaryPath);
  ...
  for(const scenario of SCENARIOS){
    ...
    connection=new RealtimeVoiceConnection({key:stepKey,voiceId:scenario.id,voiceSpokenResultGate:true,
      connect:key=>{
        ...
        ws.on('message',raw=>{try {const e=JSON.parse(raw.toString());record(t.provider,e);

if(e.type==='response.created'&&++responses>=15){stopReason='response_cap';connection.close();}}catch{}});
        const send=ws.send.bind(ws);ws.send=((data:any)=>{try{record(t.outgoing,JSON.parse(String(data)));}catch{}

return send(data);}) as typeof ws.send;
...
if(e.type==='ready')ready=true;
        ...
        if(e.type==='response_done'){
          const timer=setTimeout(()=>{playbackTimers.delete(timer);

if(!closed)connection.handle({type:'playback_done',responseId:e.responseId});},Math.max(1,(clientBytes.get(String(e.responseId))??0)/48));
...
for(const timer of playbackTimers)clearTimeout(timer);connection.close();}
