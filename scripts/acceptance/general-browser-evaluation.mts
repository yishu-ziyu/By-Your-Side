...
export async function freezeImplementation(root:string,configuration:FrozenImplementation['configuration']):Promise<FrozenImplementation>{
 ...
 const visit=async(path:string)=>{for(const entry of (await readdir(path,{withFileTypes:true})).sort((a,b)=>a.name.localeCompare(b.name))){const p=join(path,entry.name);

if(entry.isDirectory())await visit(p);else if(entry.isFile())files.push({path:relative(root,p),sha256:hash(await readFile(p))});}};
...
async function ledger(path:string):Promise<ExposureLedger>{
 try{const v=JSON.parse(await readFile(path,'utf8'));

if(v.version!==1||!Array.isArray(v.exposures))throw Error('Invalid exposure ledger');

return v;}catch(e){if((e as NodeJS.ErrnoException).code==='ENOENT')return {version:1,exposures:[]};throw e;}
}
...
async function updateLedger(path:string,update:(value:ExposureLedger)=>void):Promise<void>{
 ...
 try{const value=await ledger(path);update(value);const file=await open(temporary,'w',0o600);
...
export async function openHoldout(options:{root:string;frozen:FrozenImplementation;bundlePath:string;ledgerPath:string;developmentSites:string[]}):Promise<{bundleHash:string;tasks:EvaluationTask[]}>{
 ...
 for(const category of CATEGORIES){const group=tasks.filter(t=>t.category===category);

if(group.length<4||new Set(group.map(t=>t.siteId)).size<2)throw Error(`Insufficient category breadth: ${category}`);}
...
export async function holdoutSummary(path:string,bundleHash:string){
 const history=await ledger(path),e=history.exposures.find(e=>e.bundleHash===bundleHash);

if(!e)throw Error('Unknown batch');
 const counts={PASS:0,FAIL:0,BLOCKED:0,NOT_RUN:e.taskIds.length-e.attempts.length};

for(const a of e.attempts)counts[a.status]++;
