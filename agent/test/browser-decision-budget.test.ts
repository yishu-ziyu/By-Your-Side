...
function fixture(){const root=mkdtempSync(join(tmpdir(),'bys-decision-budget-'));roots.push(root);

return {root,file:join(root,'session.jsonl')};}

it('a task shares its durable limit across repeated tool calls and reconstructed callers',()=>{
 const f=fixture();

for(let i=0;i<16;i++)reserveBrowserDecision(f.file,'run1');
