...
describe('reading identity and cancellation', () => {
  it('replacement ignores late output and completion from the old generation', async () => {
    ...
    const old = service.run('old', transcript(), async (_t,_s,onText) => {late=onText;onText('old partial');return a.promise;}, e => events.push(e));
    const next = service.run('new', transcript(), async (_t,_s,onText) => {onText('new partial');return b.promise;}, e => events.push(e));
  ...
  it('stop is scoped to both thread and request; other pages finish normally', async () => {
    ...
    const first=service.run('r1',transcript('a'),async(_t,s,onText)=>{signal=s;onText('kept');await new Promise((resolve,reject)=>{s.addEventListener('abort',()=>reject(new Error('cancel')));void a.promise.then(resolve);});return 'done';},e=>events.push(e));
...
it('reading and handoff do not send, steer or abort the main task, and transfer imports only once', async()=>{
  ...
  const factory=vi.fn(async()=>{const runtime={session:{modelName:()=> 'test/model', answerReading:vi.fn(async(t:ReadingTranscript,_s:AbortSignal,onText:(s:string)=>void)=>{expect(t.turns[0]?.question).toBe('解释');onText('answer');return 'answer';}),importReading:vi.fn(async()=>{}),startTask:vi.fn(),abort:vi.fn()},fleet:{teamView:()=>null},handleMessage:vi.fn()};runtimes.push(runtime);return runtime;});
