...
it.each(["completed", "failed", "cancelled"])("worker %s 后自动触发全部页面移交", async reason => {
  ...
  const create = vi.spyOn(BrowserAgentSession, "create").mockImplementation(async (_rpc, s) => { sink = s;
 ...
 return worker as never; });
  ...
  try {
    ...
    else { sink.setStatus("running");

 if (reason === "failed") sink.emit({kind:"error",message:"failure"}); sink.setStatus("idle"); }
...
it("tabs action:close 自动先接管，接管失败不发关闭调用", async () => {
  ...
  const rpc = { call: vi.fn(async () => { events.push("close");
