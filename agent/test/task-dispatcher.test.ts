...
it('lets a stop overtake a correction that is still waiting for its pre-observation',async()=>{
 ...
 const steer=d.dispatch(req,'比价',async()=>{await new Promise<void>(resolve=>{releaseSteer=resolve;});return accepted;});
...
it('keeps control requests ordered on their own channel and deduplicates replays',async()=>{
 ...
 const pause=d.dispatch({...req,requestId:'c1',action:'pause'},'比价',async()=>{
  order.push('pause:start');await new Promise<void>(resolve=>{releasePause=resolve;});order.push('pause:end');return applied;});
 await new Promise(resolve=>setTimeout(resolve,0));
 const resume=d.dispatch({...req,requestId:'c2',action:'resume'},'比价',async()=>{order.push('resume');return applied;});
 const replay=d.dispatch({...req,requestId:'c2',action:'resume'},'比价',async()=>{order.push('resume:replay');return applied;});
 await new Promise(resolve=>setTimeout(resolve,0));
...
it('keeps later normal input behind an earlier control request',async()=>{
 ...
 const pause=d.dispatch({...req,requestId:'c1',action:'pause'},'比价',async()=>{
  order.push('pause:start');await new Promise<void>(resolve=>{releasePause=resolve;});order.push('pause:end');return applied;});
 await new Promise(resolve=>setTimeout(resolve,0));
 const steer=d.dispatch({...req,requestId:'s1'},'比价',async()=>{order.push('steer');return accepted;});
 await new Promise(resolve=>setTimeout(resolve,0));
 expect(order).toEqual(['pause:start']);
 releasePause();
