...
it('baseline: independent conversations have no shared admission limit',async()=>{
 ...
 const requests=Array.from({length:12},(_,i)=>dispatcher.dispatch({requestId:`r${i}`,conversationId:`c${i}`,source:'voice',action:'start',expectedRunId:null,text:`任务${i}`},`任务${i}`,async()=>{
  entered.push(`c${i}`);await barrier;

return {status:'accepted',runId:`run${i}`,message:'accepted'};
 }));
...
return {status:'accepted',runId:'run',message:'accepted'};
 }));

 try{
  const stop=await dispatcher.dispatch({requestId:'stop',conversationId:'main',source:'voice',action:'abort',expectedRunId:'run'},'当前任务',async()=>{active=false;

return {status:'applied',runId:'run',message:'stopped'};});
