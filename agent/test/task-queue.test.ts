...
function setup(directory?:string){
 ...
 const execute=vi.fn(async(r:TaskActionRequest)=>{running++;return {requestId:r.requestId,conversationId:r.conversationId,source:r.source,action:r.action,runId:r.conversationId+'-run',text:r.text!,targetTitle:r.text!,status:'accepted' as const,message:'accepted',updatedAt:1};});
