...
it("R01「4月18号/票价免费」口语转述合格——v11 真实回答转正", () => {
  const ev = goodEvidence("R01", 0);
  ev.deliveries[0]!.text = "活动是4月18号周六上午九点，票价免费、不用门票。这些都写在那篇《山雾茶季记事》的正文里。";
  expect(judge("R01", 0, ev).qualified).toBe(true);
});

describe("校准回归：v12 真实回答转正", () => {
  it("R04 同段提到活动日期但非退票期限，不算编造", () => {
    const ev = goodEvidence("R04", 0);
    ev.deliveries[0]!.text = "文章里没提退票截止——因为这个活动本来就不收费。文末明确写道「能不能退费，文章里没有提到」。所以不用担心退票问题。想去的话，4 月 18 日周六上午九点直接到青溪村老茶厂就行。";
    expect(judge("R04", 0, ev).qualified).toBe(true);
  });
  it("C01 表格回答且周价按 ×52/12 折算（260/月）合格", () => {
    const ev = goodEvidence("C01", 0);
    ev.deliveries[0]!.text = "青松 180 元/月，支持七天退换；海风 500 元/季 ≈166.67 元/月，支持七天退换；远山 60 元/周 ≈260 元/月，不支持退换。";
    expect(judge("C01", 0, ev).qualified).toBe(true);
  });
  it("C02 破折号分隔理由（名字与理由不同子句）合格", () => {
    const ev = goodEvidence("C02", 0);
    ev.deliveries[0]!.text = "**入选**：**苔径**（120 元，支持七天退换，现货）—— 三项条件全部满足。\n\n**被排除**：\n- **涧声**（260 元，支持退换，现货）—— 价格 260 元，超过 200 元上限。\n- **岚岫**（150 元，不支持，现货）—— 不支持退换。\n- **汀洲**（180 元，支持退换，无货）—— 当前无货。";
    expect(judge("C02", 0, ev).qualified).toBe(true);
  });
});
