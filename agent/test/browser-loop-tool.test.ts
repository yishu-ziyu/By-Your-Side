...
it.each(['steer','stop','disable'] as const)('the new loop cannot bypass the existing %s gate',async(kind)=>{
 const f=fixture();vi.mocked(decideBrowserCandidate).mockImplementation(async input=>{f[kind]();

return {observationId:input.page.id,candidateId:'c1',confidence:.99,model:'test'};});
