...
function progress(){const p=new TaskProgress('default');p.request('先观察X再标注，保持只处理当前页。');p.observe({type:'agent_event',event:{kind:'agent_start'}});
...
const startTask=vi.fn(()=>sink({type:'agent_event',event:{kind:'agent_start'}}));
...
if(held)sink({type:'status',state:'user'});
 ...
 it('a persisted unknown receipt never replays an already attempted write',async()=>{
  ...
  const recovered=new TaskDispatcher(new TaskReceiptStore(dir));await recovered.dispatch(req,'fixture',async()=>{writes++;

return {status:'applied',message:'done',runId:'new'};});expect(writes).toBe(1);expect(recovered.get('default','once')?.status).toBe('unknown');
 });
});
