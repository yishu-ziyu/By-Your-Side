...
function setup(steer?: (text: string, startedAt: number | null) => Promise<void>, route?: ConstructorParameters<typeof StepVoiceSession>[0]["route"], diagnostic?: ConstructorParameters<typeof StepVoiceSession>[0]["diagnostic"], extra?:Pick<ConstructorParameters<typeof StepVoiceSession>[0],"getTargets"|"receiptAudioCache">) {
  ...
  const configure=()=>{const update=socket.sent.filter(e=>e.type==='session.update').at(-1);
...
it("ends old leases and ignores late credential resolution/foreign conversation commands", async () => {
  ...
  const service = new VoiceService(() => ({} as TaskProgressSnapshot), vi.fn(), () => key, () => {
    const s = { start: vi.fn(), close: vi.fn(), command: vi.fn() }; created.push(s);

 return s as unknown as StepVoiceSession;
  });
...
it("late routed intentions are invalidated on a new voice turn", async () => {
  ...
  const h=setup(undefined,async(_text,_startedAt,stillCurrent)=>{current=stillCurrent;

return new Promise<{kind:"none"}>(r=>{finish=r;});});
...
it('keeps newer speech during reconnect and invalidates the older route before reconnect completes',async()=>{
 ...
 const route=vi.fn((_text:string,_at:number|null,valid:()=>boolean)=>{current=valid;
...
it('stops after three failed reconnects even when the last healthy connection was over a minute ago',async()=>{
 ...
 const session=new StepVoiceSession({getSnapshot:()=>null,emit:e=>events.push(e),connect:()=>{const s=new Socket();sockets.push(s);
...
it.each(['asr','instructions'])('keeps a bounded %s wait after reconnect',async(stage)=>{
 ...
 const session=new StepVoiceSession({voiceId:'bounded',getSnapshot:()=>({conversationId:'A',runId:'run',observedAt:1,state:'running',goal:null,startedAt:1,active:[],lastAction:null,successVerified:false}),route,emit:e=>events.push(e),connect:()=>{const s=new Socket();sockets.push(s);
...
it('replays the accepted receipt when connection is lost after generation but before playback ends',async()=>{
 ...
 const session=new StepVoiceSession({getSnapshot:()=>({conversationId:'A',observedAt:1,state:'running',goal:null,startedAt:1,active:[],lastAction:null,successVerified:false}),route,emit:e=>events.push(e),connect:()=>{const s=new Socket();sockets.push(s);
