...
export function skillBrowser() {
  ...
  const call = vi.fn(async (name: string, params: Record<string, unknown>, _timeout?: number, _session?: string, _program?: string, _epoch?: number, id?: string) => {
    ...
    if (name === "fill") { node.value = String(params.value); writes.push({ name, target: node.attrs["aria-label"], value: params.value }); state.afterFill();

 return { filled: true }; }

    if (name === "click") { output.textContent = `${query.value} / ${region.value}`; writes.push({ name, target: node.attrs["aria-label"] });
