...
const stream = (_model: unknown, context: any) => {
 ...
 if (invokeTool) { yield { type: "toolcall_start", contentIndex: 0, partial: message }; yield { type: "toolcall_delta", contentIndex: 0, delta: "{}", partial: message }; yield { type: "toolcall_end", contentIndex: 0, toolCall: message.content[0], partial: message }; }

 yield { type: "done", reason: invokeTool ? "toolUse" : "stop", message }; }, result: async () => message };
};
...
const action = defineTool({ name: "marker_action", label: "Marker action", description: "Record one local external action", parameters: Type.Object({}), execute: async () => { writeFileSync(actionFile, String(actionCount() + 1));

 return { content: [{ type: "text", text: "action complete" }], details: {} }; } });
