...
describe('explicit user delivery boundary',()=>{
 ...
 it('does not accept another worker, conversation or run as the lead delivery',()=>{const h=harness();

for(const [record,extra] of [[h.delivery(),{sessionId:'worker-1'}],[h.delivery({conversationId:'other'}),{}],[h.delivery({runId:'stale-run'}),{}]])h.emit({kind:'user_delivery',delivery:record},extra);expect((h.p.snapshot().conversationContext as any).latestDelivery??null).toBeNull();});
