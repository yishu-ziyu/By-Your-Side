...
function fixture(){
 ...
 invokeDisplayTool:vi.fn(async()=>{order.push('loop');return {details:{status:'needs_verification',reason:'verify',receipts:[],modelCalls:1,decisions:[]}};}),
...
it('does not appear idle between a saved-skill miss and the general loop',async()=>{
 ...
 f.host.invokeDisplayTool.mockImplementation(async()=>{expect(f.host.displayWork).toBe(pending);return {details:{status:'needs_verification',receipts:[],modelCalls:0,decisions:[]}};});
...
it('a cancelled preparation cannot start a late reasoning-model turn',async()=>{
 ...
 f.host.readUserPageForPrompt.mockImplementation(async()=>{f.host.controlEpoch++;return 'late page';});
...
it('does not start the reasoning model after cancellation during the initial loop',async()=>{
 const f=fixture();f.host.invokeDisplayTool.mockImplementation(async()=>{f.host.controlEpoch++;return {details:{status:'cancelled',receipts:[]}};});
 await f.run();expect(f.session.prompt).not.toHaveBeenCalled();
});
