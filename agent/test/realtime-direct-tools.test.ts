  ...
  const transport=vi.fn(async frame=>{
    ...
    if(frame.name==='read_element'){
      if(frame.params.readback){
        ...
        if(page.readbackMode==='failure'){rpc.handleResult(frame.id,false,undefined,'read unavailable','not_executed');return;}
        if(frame.params.readback.documentId!==page.documentId){rpc.handleResult(frame.id,false,undefined,'READBACK_DOCUMENT_CHANGED','not_executed');return;}
      }
      try {
