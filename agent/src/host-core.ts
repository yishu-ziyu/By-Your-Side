  ...
  return {
    ...
    sendHelloOk(conn) {
      void session.availableModels().then((models) => {
        conn.send({ type: "hello_ok", version: PROTOCOL_VERSION, model: session.modelName(), models, hostVersion: HOST_VERSION, extensionVersion: "0.1.0", storageSchema: STORAGE_SCHEMA_VERSION, ...options.helloExtras?.() });
