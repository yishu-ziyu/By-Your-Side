...
export class SkillLearningTrace {
  ...
  observe(event: SkillEvidence): ToolContract["read_element"]["params"] | void {
    ...
    if (this.ids.length >= 80) { this.invalid = true; return; }
    if (event.error) {
    ...
    if (tabId !== undefined && tabId !== this.run!.context.tabId) { this.invalid = true; return; }
    const documentId = event.target?.documentId ?? data?.documentId;
    if (documentId) {
      if (this.documentId && this.documentId !== documentId) { this.invalid = true; return; }
    ...
    this.proof = null;
    if (this.steps.length >= 20 || !["fill", "click", "press_key"].includes(event.name)) { this.invalid = true; return; }
    if (event.name === "press_key") {
      if (event.params.key !== "Enter" || this.steps.at(-1)?.anchor?.inputType !== "search") { this.invalid = true; return; }
      this.steps.push({ at: Date.now(), kind: "press", key: "Enter" }); this.ids.push(event.toolCallId); return;
    }
    const source = event.target?.anchorSource;
    if (!source || !event.target?.documentId || isSensitiveField(source)) { this.invalid = true; return; }
    const anchor = anchorFor(source);
    if (!anchor.name || forbiddenInSkill(JSON.stringify(anchor))) { this.invalid = true; return; }
    if (event.name === "click") {
      if (!SAFE_CLICK.test(anchor.name) || (event.result as { held?: boolean })?.held) { this.invalid = true; return; }
      this.steps.push({ at: Date.now(), kind: "click", anchor });
    } else {
      const value = event.params.value;
      if (typeof value !== "string" || !value || value.length > 500 || /[\r\n]|sk-[A-Za-z0-9]|Bearer\s|password\s*=/i.test(value)) { this.invalid = true; return; }
      if (this.steps.some(step => step.kind === "type" && JSON.stringify(step.anchor) === JSON.stringify(anchor))) { this.invalid = true; return; }
  ...
  finish(completedRunId: string | null, completed: boolean): SkillCandidate | null {
    ...
    if (!autoSkillEligible(skill)) return null;
