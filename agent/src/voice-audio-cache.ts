...
export class VoiceAudioCache {
 ...
 private allowed(text:string){let rest=text.replaceAll('指定会话：','');

for(const phrase of STATIC_SPEECH)rest=rest.replaceAll(phrase,'');

return text.length>0&&text.length<=180&&rest==='';}
 get(text:string):string[]|undefined{const key=this.key(text),audio=this.entries.get(key);
...
return audio?.slice();}
 put(text:string,audio:string[]):void{
  ...
  this.entries.set(this.key(text),audio.slice());
