...
if(typeof p!=='number'||!Number.isFinite(p)||p<0||p>1)return fallback('invalid_response',partialObserved);
 }
...
export async function decideDisplay(request:string,signal:AbortSignal):Promise<DisplayRoutingResult>{
  ...
  try{
    ...
    if(!response.ok){log(`http_${response.status}`);

return {kind:'fallback',reason:`http_${response.status}`};}

    let raw:any;

    try{raw=await response.json();}catch{log('invalid_response');

return {kind:'fallback',reason:'invalid_response'};}

    if(signal.aborted)return {kind:'cancelled'};

    if(Date.now()-started>=1000){log('late_response');

return {kind:'fallback',reason:'late_response'};}
