import { bindSkillInputs, normalizeSkillHost, skillHealth, type SkillRun } from "../../shared/skill.js";
...
export async function trySkillFastLoop(options: {
  ...
  current(): boolean;
  execute(name: "snapshot" | "browser_run", params: Record<string, unknown>): Promise<{ details?: unknown }>;
  ...
  try {
    ...
    if (selected) {
      const outcome = { at: Date.now(), ok: false, steps: 0, elapsedMs: 0, error: reason };
      finishSelection(outcome);
      return options.current() ? { kind: "done", outcome, skillName: "已保存的技能" } : stop();
    }
    return options.current() ? { kind: "fallback", reason } : stop();
  } finally {
