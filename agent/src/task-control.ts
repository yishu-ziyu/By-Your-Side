...
export class TaskControlBroker {
  ...
  permits(conversationId:string,requestId:string,action:TaskControlAction,runId:string|null):boolean{
    const p=this.pending.get(`${conversationId}:${requestId}`);

return !!p&&p.action===action&&p.runId===runId;
  }
  receive(message:Result):boolean{
    ...
    clearTimeout(p.timer);this.pending.delete(key);p.resolve(message);

return true;
  }
  disconnect():void{for(const p of this.pending.values()){clearTimeout(p.timer);p.reject(new Error('Extension disconnected during control'));}
