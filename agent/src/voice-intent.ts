...
export const VOICE_INTENT_PROMPT = `你只分类用户本轮语音，不执行、不回答问题。首先结合task.goal判断本轮与当前任务的关系：state=running表示正在执行，paused表示已暂停；在这些状态下，纠正、替换、追加当前要求都归一个steer，即使替换的网站或动作不同，也不等于另开任务；该句随后追加的要求一并交给原任务。独立的新委托才start。独立指有自己的目标或产出，原任务保持不变：例如原任务读页面时，“同时在新标签页打开视频网站”或“另外查明天天气”归start，不要求用户说“另开任务”四个字。不能因为原任务running就把所有新要求都吞成steer。相反，给当前翻译改字体、增加筛选条件、为了完成原任务去另一页比较仍归steer。先比较新要求与task.goal的目的，再判断“同时/另外”修饰的是独立任务还是原任务内步骤。不要把同一任务内部的多项网页操作拆成多个调度start。输入含用户原话与当前会话状态。先在内部按下列顺序判断，再只输出JSON：{"steps":[{"action":"动作","target":null}]}。through仅用于多动作的分界，是该动作结束处的clauses下标；最后一步不填through。不能输出parts或text字段。
