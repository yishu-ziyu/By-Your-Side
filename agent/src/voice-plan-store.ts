...
export class VoicePlanStore {
 ...
 private read(key:string):Record|undefined{
  ...
  try{const r=JSON.parse(readFileSync(join(this.directory,`${key}.json`),'utf8'));

if(!r||typeof r.id!=='string'||typeof r.fingerprint!=='string'||!Array.isArray(r.steps))throw Error('Invalid voice plan');

return r;}
  catch(e){if((e as NodeJS.ErrnoException).code==='ENOENT')return;throw e;}
 }
 private write(key:string,record:Record,claim=false):void{
  if(!this.directory){if(claim&&this.memory.has(key))throw Error('Plan exists');this.memory.set(key,structuredClone(record));

return;}

  const file=join(this.directory,`${key}.json`);

  if(claim){writeFileSync(file,JSON.stringify(record),{flag:'wx',mode:0o600});
...
return;}
 ...
 update(source:string,id:string,change:{steps?:VoicePlanStep[];proposal?:VoiceProposal}):void{
  const key=this.key(source,id),record=this.read(key);

if(!record)throw Error('Plan not claimed');this.write(key,{...record,...change});
 }
 ...
 get(source:string,id:string):VoiceRouteResult|undefined{try{const record=this.read(this.key(source,id));

return record?this.result(record):undefined;}catch{return uncertain();}}
...
if(active)return active.fingerprint===fingerprint?active.promise:Promise.resolve(conflict());
  ...
  const promise=Promise.resolve().then(async()=>{
   ...
   try{
    ...
    this.write(key,{...record,result:final});
...
return final;
  ...
  this.active.set(key,{fingerprint,promise});void promise.finally(()=>this.active.delete(key)).catch(()=>{});
