...
export class TaskQueue {
 ...
 constructor(private readonly options:{
  ...
  try{
   ...
   for(const job of records){
    if(!isTaskActionRequest(job?.request)||!isTaskReceipt(job?.receipt)||!['queued','suspended','starting','running','completed','cancelled','failed','unknown'].includes(job.state))throw Error('Invalid task queue entry');
    // A restarted host must not repeat an external action or silently resume an old page request.
    if(job.state==='queued'){
     job.state='suspended';job.receipt={...job.receipt,status:'queued',message:'连接已重启，这项待办尚未启动；原要求已保留，明确继续后才重新安排。'};
 ...
 add(request:TaskActionRequest,title:string):TaskReceipt{
  ...
  if(this.list().filter(j=>j.state==='queued'||j.state==='suspended').length>=this.options.maxWaiting)return {...base,status:'rejected',message:'待办已满，这项要求尚未接收；可以先取消一项待办。'};
 ...
 cancel(id:string):TaskReceipt|undefined{
  const job=this.get(id);if(!job||!['queued','suspended'].includes(job.state))return;
 ...
 suspendPending():void {
  let failed=false;
  for(const job of this.jobs.values())if(job.state==='queued'){
   const change:Partial<QueuedTask>={state:'suspended',receipt:{...job.receipt,message:'连接断开，这项待办尚未启动；明确继续后才重新安排。',updatedAt:Date.now()}};
   try{this.update(job,change);}catch{Object.assign(job,change);failed=true;}
   this.options.changed(job);
  }
  if(failed)throw new Error('Suspended pending jobs in memory, but could not persist all of them');
 }
 resumePending(id:string,context?:PageContext):TaskReceipt|undefined {
  const job=this.get(id);if(!job||job.state!=='suspended')return;
