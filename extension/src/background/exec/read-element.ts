...
async function readDom(tabId: number, target: ReturnType<typeof parseTarget>, member: string, properties: ElementProperty[], readback?: {documentId:string;deadline:number}, check=()=>{}): Promise<{data:ElementData;documentId?:string}> {
  ...
  const results = await chrome.scripting.executeScript({
    ...
    func: readInPage,
    args: [target.kind, target.kind === "ref" ? target.ref : null, target.kind === "css" ? target.selector : null, properties, null, readback??null],
  });
  check();
  ...
  const read = async (): Promise<{data:ElementData;documentId?:string}> => {
    ...
    if (target.kind === 'ref') {
      if (isAxRef(tabId, target.ref)) {
        const observed=await withObservedDocumentIdentity(tabId,executionKey,()=>readAxRef(tabId,target.ref,properties,readback,check),check);
        check();
