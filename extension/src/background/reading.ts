...
export function installReading(deps: Dependencies) {
  ...
  chrome.runtime.onMessage.addListener((raw, sender, respond) => {
    ...
    void (async () => {
      ...
      if (raw.type === 'reading_leave') { for (const r of records.values()) if (ownedDocument(r)) stop(r); return {ok: true}; }
      ...
      if (raw.type === 'reading_stop') { stop(record); return {ok: true, record}; }
      ...
      if (raw.type === 'reading_handoff') {
        ...
        if (record.transferredConversationId) { opening = false; deps.select(record.transferredConversationId); return {ok: true, record}; }
  ...
  const dropTab = (tabId: number) => {
    void ready.then(() => { for (const [key, r] of records) if (r.source.tabId === tabId) {stop(r); records.delete(key);} persist(); });
  };
    ...
    receive(event: ReadingEvent) {
      void ready.then(() => {
        ...
        if (record && record.handoffRequestId === event.requestId && event.state === 'error') {
          opening = false; record.handoffError = event.error ?? '无法转到侧栏，请重试。'; record.handoffRequestId = undefined; publish(record); return;
        }
