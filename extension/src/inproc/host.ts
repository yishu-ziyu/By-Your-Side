...
const runtime = createModelRuntime((providerId, credential) => { port?.postMessage({ type: "inproc_credential", providerId, credential: credential ?? null }); });
...
chrome.runtime.onConnect.addListener((next) => {
