...
export function mountVoiceUI(composer: HTMLElement, getConversation: () => string, send: (m: ClientMessage) => boolean, getInput: () => VoiceInputContext = () => ({}), diagnostic?: (event: string, fields: Record<string, string | number | boolean | null | undefined>) => void) {
  ...
  const diagStart = node('button', 'voice-diag-start', '开始录音') as HTMLButtonElement;
  diagStart.type = 'button';
  const diagStop = node('button', 'voice-diag-stop', '结束录音') as HTMLButtonElement;
  diagStop.type = 'button';
  diagStop.disabled = true;
  const diagExport = node('button', 'voice-diag-export', '导出JSON') as HTMLButtonElement;
  diagExport.type = 'button';
  const diagClear = node('button', 'voice-diag-clear', '清空') as HTMLButtonElement;
  diagClear.type = 'button';
  ...
  loadC0.type = 'button';
  const loadC1 = node('button', 'voice-diag-load-c1', '载入C1') as HTMLButtonElement;
  loadC1.type = 'button';
  const playerC0 = node('audio', 'voice-diag-player-c0') as HTMLAudioElement;
  playerC0.controls = true;
  playerC0.preload = 'none';
  const playerC1 = node('audio', 'voice-diag-player-c1') as HTMLAudioElement;
  playerC1.controls = true;
  playerC1.preload = 'none';
  ...
  function renderDiag(): void {
    ...
    if (diagNotice && Date.now() - diagNotice.at < 5000) {
      ...
      diagnosticStatus = '正常语音自动记录；手动复现未开启（点“开始录音”单独复现一条，期间不执行网页操作）。';
    } else if (!log.isConfirmed) {
      diagnosticStatus = '等待服务端确认诊断模式…（确认前不会发送任何音频）';
    } else if (client.diagnosticRecording) {
      diagnosticStatus = `录音中：${latest ? (latest.samples / latest.sampleRate).toFixed(1) : '0.0'} 秒 / 上限 ${log.limitSeconds} 秒`;
    } else {
    ...
    if (!items.length) {
      ...
      for (const capture of items.slice(-4).reverse()) {
  ...
  diagExport.onclick = () => {
    ...
    link.href = url;
    link.download = `voice-diagnostic-${Date.now()}.json`;
    link.click?.();
    URL.revokeObjectURL?.(url);
  };
  diagStart.onclick = () => {
  ...
  window.addEventListener('focus', () => {
    if (phase === 'error' && client.needsMicrophonePermission) {
      void microphonePermissionState().then(state => {
        if (state === 'granted' && phase === 'error') {
          client.needsMicrophonePermission = false;
          end.textContent = '重试';
          status.textContent = '麦克风已授权，点击重试。';
        }
      });
    }
  });
  ...
  return {
    ...
    receive: (m: Extract<ServerMessage, {
      type: 'voice';
    }>) => client.receive(m),
    deliver: (delivery: {
