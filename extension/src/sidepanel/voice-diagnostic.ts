...
export class VoiceDiagnosticLog {
  ...
  noteCapture(turn: number, code: string, detail?: string): void {
    ...
    if (!capture) { this.note(code, detail);
 ...
 return; }
  ...
  captureFrame(pcm: Int16Array): 'ok' | 'full' | 'ignored' {
    ...
    if (remaining <= 0) { capture.capped = true; this.notify();
 ...
 return 'full'; }
  ...
  apply(record: VoiceDiagRecord): void {
    if (record.type === 'ready') { this.confirmedBy(record);

 return; }

    const capture = this.captureFor('turn' in record ? record.turn : null);

    if (!capture) { this.looseNotes.push({ code: `orphan_${record.type}`, at: Date.now() }); this.notify();
