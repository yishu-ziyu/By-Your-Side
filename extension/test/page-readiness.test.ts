...
const executeScript=vi.fn(async()=>{const d=documents[Math.min(i++,documents.length-1)]!;

return [{documentId:d.id,result:{readyState:d.state,url:d.url??'https://example.test/'}}];});

 vi.stubGlobal('chrome',{scripting:{executeScript},tabs:{get:vi.fn(async()=>({url:'https://example.test/',status:'loading'}))}});
