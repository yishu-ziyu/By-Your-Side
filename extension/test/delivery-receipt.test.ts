...
function firstUserEntry(sent: BgToPanel[]): PanelHistoryEntry {
  for (const e of sent) {
    if (e.kind === "history") {
      const entry = e.entries.find((x) => x.item.kind === "user");
...
describe("background 面板路由 × 上行不可用（issue #4）", () => {
  ...
  it("user_message：background 层接受（history 回显）后上行失败要回执 delivery ok:false", async () => {
    ...
    const entry = firstUserEntry(port.sent);
    expect(entry.item).toEqual({ kind: "user", text: "E2E 普通问题" });
    ...
    await vi.waitFor(
      () => {
        expect(port.sent).toContainEqual({
          ...
          seq: entry.seq,
          ok: false,
          original: { type: "user_message", text: "E2E 普通问题" },
        });
      },
      { timeout: 3_000 },
    );
    ...
    await vi.waitFor(
      () => {
        expect(port.sent).toContainEqual({
          ...
          seq: entry.seq,
          ok: false,
          original: { type: "steer", text: "E2E 插话", context },
        });
      },
      { timeout: 3_000 },
    );
