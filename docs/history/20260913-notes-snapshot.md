> 历史快照，保存至2026-09-13仓库整理前的全部内容。包含已过期状态，不作为当前结论；当前入口见[STATUS](../STATUS.md)。

# 会话工作笔记

## 真实路径验收继续方式（2026-09-13）

用户要求用原生电脑控制核实产品已实现承诺。本轮修复收尾、链接和重复呈现后，原生同题复验已完成；后续优先沿[功能承诺清单](../evals/20260913-feature-claims.md)补交还/运行中插话/会话隔离等未验路径。不要把有入口、旧合成测试或本轮三个修复的通过，替换成整个产品已通过。当前状态与证据只维护在[STATUS](../STATUS.md)。

## 页面移交、经验与授权的验证边界（2026-09-13）

用户批准的三项修复均有定点反例。进度统一见 [STATUS](../STATUS.md)，前后证据见[验收](../evals/20260913-architecture-closures.md)。原 WIP 基线与执行回执在 `/tmp/ego-fixes-20260913/`；不要把相对 HEAD 的全部差异算成本轮。

本轮最有用的验收接缝是生产装配：孤立的“必须有票据”测试通过，并不证明侧栏有真实入口。授权检查现连接实际 Manager/runtime/tools、编译面板与 Background，再由本地 HTTP 计数验允许/拒绝。模型与 native 传输是替身，仍不能代替日常语音或真人体验。

页面移交不能整体停止原成员来省事，因为同一成员可能还在操作别页。按 tabId 关闸，独立验证原成员另一页能继续。授权不能按会话顺序 id 绑定：broker 重建会让旧确认命中新请求；现用随机 UUID，原参数仍留在运行时。其他细节由源码、测试和验收解释，不另复制状态。

## 语音纠正确认的跨模块缺口（2026-09-12）

原复述确认测试与插话预观察测试各自通过，但只证明相应代码存在；没有覆盖“在 A 页纠正、到 B 页确认”后参数能否送达。确认丢掉 context，导致已存在的预观察函数实际不运行。本轮增加真实 Manager → BrowserAgentSession 的行为检查及隔离浏览器原页检查，替换受影响的确认载荷源码字符串断言。进度与证据见 [STATUS](../STATUS.md) 和[本轮验收](../evals/20260912-voice-confirm-context.md)。

## 正式版收敛（2026-09-11）

- 第二轮 live 表单 84% 的主因：单人页调用 `page_operation` 被拒后账本记 `unknown`，闸门把 snapshot/read_element 也拦住，同错三次停手。`three-pages` 是同一「下一页」第二次被当重复点击。第三轮改完后 `10-11-47` **145/150 = 96.7%**（表单 96% / 多步 94% / 读取 100%），门槛过了，不覆盖前两轮。剩余 5 次主要是过期 ref 仍记 unknown；已补动作前拒绝 `not_executed` 和 live 回传 executionFact，未再跑 150。
- 执行包下一步：PR-03 播放队列迁完，然后 PR-05 结果卡接终态。不打正式标签。开发日志 [20260911-07](../devlog/20260911-07-三轮live才过表单门槛.md)。
- PR-03/05 本轮：`VoicePlayback` 管排队与打断；侧栏 `#task-result-card` 在正式交付后出现。证据 [voice-playback-and-result-card](../evals/20260911-voice-playback-and-result-card.md)。下一步是 PR-04 事件循环/堆，或你来看侧栏结果卡。
- PR-04 B01：`list` 热路径改走内存；20k 回执 + 200 会话事件循环 p95=1.43ms。B02 满 2 小时 `13-16-44` 堆增量 -0.56MiB，RSS 末值 24MB；无强制 GC、无语音。证据 [pr04-scale-loop](../evals/20260911-pr04-scale-loop.md)。
- PR-05 机器项：隔离无头 12 档尺寸不横溢、2000 条历史上输入 p95=34ms、Enter 发送、Escape 关会话菜单。证据 [pr05-panel-layout](../evals/20260911-pr05-panel-layout.md)。人眼与 U02/C01 native 未做。
- U02/C01：隔离真 SW+面板 `15-39-12` 交付可见 p95=18ms、接管关闸 p95=15ms。打开进入未定会话时仍放行本次正式交付；Agent 未连接时本地仍提交接管、不再撤闸。证据 [native-latency](../evals/20260911-native-latency.md)。开发日志 [20260911-08](../devlog/20260911-08-规模侧栏和接管关闸.md)。

- 评测门槛只从 `eval/protected/quality-gates.json` 读；SHA `61f0bab1557be3330315214057466cb889b23b02d133110cf13fc3544502e748`。候选改这个文件会被 `release:verify` 拒绝。
- GitHub `main` **未保护**，没有 CODEOWNERS，没有独立留出集保管人。不能写「评测已独立保护」。
- fetch POST 现在要 ControlGate + 授权票据。测试 loopback 必须 `installFetchTestOrigin`，生产永远不要调。
- 默认语音 `start` 不再带 `capture:true`。`VoiceCaptureStore.begin(..., {persistAudio:true})` 才写 WAV。
- 回执目录会出现 `_index.json`；扫描/破坏测试必须跳过它，否则会改错文件。
- 打开面板即新会话的行为不要改回去。README 已改掉旧「恢复选中会话」句。
...
### 全功能真实路径续验（2026-09-13）
原生控制不能和另一个Chrome操作者并发验接管/纠正；本轮多次落在红鲱鱼/秘塔窗口，不能归因于插件。恢复时继续用已启动的127.0.0.1:48765测试设施，按feature-journeys固定用例跑。事件条数不是Agent操作数，必须与人工时间和侧栏过程对应。教学存在文案/提示边界冲突；看我做记录与编译重放需分别验。

### 真人语音确认循环（2026-09-13）
用户已按要求实际说了三句，不要让用户重复承担诊断。重复模板的直接原因是复合肯定未识别又回退分类器；只扩大词表不够，无有效pending的明确确认还要防止确认自身，同时不能阻断无pending“继续”。原页另有旧run release卡住新接手，已分开修复并文字复测。当前扩展已加载，两条链的证据在voice-confirm-loop；真人重测未冒充通过。示范技能修复机器通过并补命名优先级遗漏，真正再次录制/编译/重放仍未验。测试记忆永久忘记的现场问题尚待用户答复，不能自行删除。

### 任务缺省页与控制状态（2026-09-13）
默认页必须一路到真实输入执行器：仅让snapshot/read_element带tabId并不能保护click/fill，且截图handler曾把参数丢成{}。当前RPC在授权和takeTab前补目标，执行器沿用原归属闸门，明确关闭的目标不回落活动页。点击跟随新标签也须更新默认页。

控制条来源不等于页面归属：非成员活动页也能展示某会话的接管条，按钮需路由到实际绘制的owner；按会话清理，逐页绘制串行读取最新期望状态，防迟到旧条复活。task run与同run交还要区分，旧aborted team不能盖住新任务。

原生控制每次完整AX都会重编element ID；在保存AX后再用旧ID点击会造成错误或无效操作。所有动作从同一份最新AX重新取索引。截图和AX仍可能延迟，结合操作前后状态与实际任务trace，不能把旧图当新证据。最终验收记录见interaction-loop-audit；真实语音未重测，持久记忆删除仍未获现场答复。


2026-09-13 同类交互排查收尾：跨页提示竞态的外层当前性检查不足，必须覆盖 ensureCursor 后及写入顺序；页面共用提示的 clear 必须按 owner 撤回，不能无条件 hide。这次两条追加失败推翻初修通过，主代理按逐页意图与队列修正。原生等待→切页→中止已验提示消失；草稿恢复由 DeepSeek 部分执行后主代理完成。证据及未验边界以 interaction-loop-audit 为准。
