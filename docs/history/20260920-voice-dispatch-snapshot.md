> 历史快照：2026-09-20 文档整理前的 `docs/voice-dispatch.md`。保留当时文字及矛盾，不作为当前状态或执行授权。当前入口：[STATUS](../STATUS.md)。仅调整相对链接以保持可访问。
...
commit先提交音频，可携带PageContext和图片附件；异步页面补充通过contextPending/input_context按原turn绑定。仅换轮不会丢掉旧片段，换会话或语音连接失效才拒绝旧来源；派发仍等待上下文就绪。草稿正文不拼入语音指令。
...
运行中明确的独立要求直接登记，复用独立执行会话，不要求用户先另开会话。当前最多2个根任务执行、8个独立要求等待；同来源页冲突等待，明确新标签页可独立执行。结果按任务身份回到原语音。待办可按名称修改、取消；重启保留记录但不自动重做。旧版已保存的另开会话提案仍兼容90秒内同连接确认。详见[多要求调度](../voice-multi-request-design.md)。

复合动作按序接收，前一步失败即停止后续。原话片段不能交叉、倒序或重叠。已接收的步骤不能因后来插话被声称撤回。真实复合恢复路径尚未通过当前版本验收。

## 新增只读页面问答

observe是语音意图，不是TaskAction。空闲时的读页问答交给现有主Agent任务路径；已有任务运行时，可以走独立的只读页面观察：以提交时的tabId读取snapshot和screenshot，再将实际内容交给多模态模型，不在这条只读路径提供写工具。网页和图片里的指令当数据处理。
