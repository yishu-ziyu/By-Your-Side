...
- 逗号切句的产品侧处理（用户 2026-09-25 选：两条修法都做 A/B、问句合并显示）：
  - A/B 同一时段各 5 次（[产物目录](../../out/acceptance/real-path/)，`04-2x`–`04-37`）：现状 2/5；①「回复说要做却没调用就补一次」1/5，另出现一次没圈却说「完成，已圈出来」，不采用；②「页面操作交给文字 agent」0/5，语音模型从不调 `task_action`，只读页面后回答，不采用。只装扩展时 Jev 是空实现，①的原型用文字匹配判断，本就只为量「补一次有没有用」。
  - 记下工具参数后查明主因：前半句的 snapshot 被作废，模型改用 `read_elements {selector:"button"}`，返回里没有单个元素的定位，只好 `mark loc=css:button`，命中两个按钮报错后停下。改为 `read_elements` 每个元素带唯一 `target`（`loc=css:#save`）：同一时段 5/5（对照 2/9），[产物](../../out/acceptance/real-path/2026-09-25T04-46-59-629Z-inproc-voice-mark/result.json)。
  - 侧栏「你：」合并显示：第一版只看侧栏状态，没生效——晚到的前半句带原轮次，被客户端按「旧轮次」丢了。改为客户端放行上一轮的用户转写、显示层按「最后一段之后助手是否回答过」决定拼接；之后显示「你：帮我把保存按钮圈出来。不要点它。」（[产物](../../out/acceptance/real-path/2026-09-25T04-50-35-750Z-inproc-voice-mark/result.json)）。
  - 作废理由改为如实说明并让模型重新调用：交替对照 10 次里 6 次是服务端卡住超时；跑完的 4 次现状 1/2、新文案 2/2，真正走到作废分支的只有 1 次（成功）。复核时发现第一版并没生效：新一轮开口会先中止在途调用，判定条件却排除了「已中止」，模型收到的仍是旧文案；改为只看轮次后，2 次都收到新文案，1 次接着圈对，1 次仍说「我先读一下页面」就停。证据薄，保留理由是旧文案与事实不符；模型「说要去做就停」没有根治。
  - 怀疑 B 的 4 倍速补发导致服务端卡住：交替对照有 B 1/5、无 B 2/5，卡住或不响应有 B 1 次、无 B 2 次，不成立，B 保留。
  - 卡住时宿主主动提交（用户 2026-09-25 同意试）：宿主按发出音频的音量判断，人声停 1.5 秒仍无「说完」、或听到 0.8 秒以上人声却 2.5 秒无任何事件，就发 `input_audio_buffer.commit`，2.5 秒无回复再发 `response.create`。8 次（[产物](../../out/acceptance/real-path/2026-09-25T06-09-10-539Z-inproc-voice-mark/result.json)）：真卡住的 2 次，服务端对提交和 `response.create` 都没有任何回应，约 60 秒后断线，说明是整个会话挂住而不只是断句；其余几乎每次都误触发（服务端事件本来就比人声晚 1–2 秒），2 次切在两段话中间只听到前半句、圈画失败。不采用，补丁已撤。
  - 挂住的快速发现（用户 2026-09-25 选「快速发现 + 请用户重说」，不补发旧音频）：宿主累计还没得到回应的人声（≥0.6 秒）或未配对的「开始说话」，距最后一次人声和最后一个服务端事件都超过 12 秒，就发带 `sayAgain` 的可恢复错误并断开，侧栏重连后显示「刚才语音服务没有响应，请再说一遍」。连接层单测 `agent/test/realtime-stall-recovery.test.ts` 先红后绿（8 条，含实测漏判的两种形态）。真实路径修正过两次：第一版见到前一段的转写就清零，漏判「后半句只有开始说话」；第二版被挂住期间仍每秒两三条的 `transcription.delta` 一直刷新，拖到 46–73 秒才发现。改后顺序跑 6 次（[产物](../../out/acceptance/real-path/2026-09-25T07-13-46-258Z-inproc-voice-mark/result.json)）：挂住 4 次都在 22.1–23.3 秒发现、26–27.6 秒重连并提示重说（原先 60–100 秒无提示）；没挂住的 2 次都圈对、问句完整，无误触发。期间有一段两组测试重叠运行（代理失误），那几次的挂住率可能偏高。
  - 读页后停住：能直接操作页面时 `read_page` 结果附 `next` 提示（没有元素引用，要操作就调 snapshot）。之后的圈画里模型都直接走了 snapshot，提示没被用到，效果未验证；问答 2/2 照常答对，没干扰读页回答。
  - 现在的主要失败是服务端卡住：开始说话后不再回任何事件，约 60 秒后断开重连，下午 10 次里 6 次；还见到把「不要点它」识别成「要点它」（未点击）、吞掉「帮我把保存按钮」。未处理。
