...
用户在核对独立成果后同意集成判断。后续明确大量执行交 DeepSeek、主代理与 Sol 负责规划和审查；本轮已按该分工执行，worker 无法启动的隔离浏览器由主代理在已有权限下完成。来源提交 `52b593e`，集成基线 `c57edb9`；原始实验与失败记录见 [原验收](20260919-skill-fast-loop.md)。本轮不把原五项改记为全部通过。

## 完成标准

- [x] 保存来源成果，保留主线 T01/T02 及并行 T03–T05 的工作。检查者：主代理核对 Git 差异与祖先。
- [x] 路由缺材料、超时、取消与误匹配边界得到复现和核验；不降低执行阈值换取命中率。检查者：定点单测、真实 Jev 原集合。
- [x] 隔离无头侧栏完成首次任务、确认保存、自动及手动换材料、停止与改口；最终认证版本改动后复验首次/保存/自动与额外交付，手动及控制复用前一份证据（边界详见下表）。检查者：主代理、真实模型与生产浏览器验收器。
...
- [x] 本地模板只在每个槽位有成对引号时才确定性直达；尾部动作/条件不再被吞。检查者：`agent/test/skill-router.test.ts`。
- [x] 最终差异复核并合入本地主线。检查者：主代理（快进至本文件提交；未推送、未重载）。

## 当前核验结论（2026-09-19，以下优先于历史执行记录）

可合入本地主线作为已验证的技能闭环；不代表原五项全部达标，不是日常扩展发布。本轮保留主线 `2b2392a` 的 T03–T05 与 T06 记录，集成提交为 `81497b5`、`526ed9d`，最后修复与证据见本文件所在提交。

| 验证 | 实际结果与证据 |
| --- | --- |
| 最终源码工程检查 | `check-contract-version.log` + `.exit`：exit 0，2404 单测 + 2 规模测试，类型、模块边界、构建通过；目录 `out/acceptance/skill-loop-integration-verification/` |
| 最终源码：首次→保存→自动换材料 | `out/acceptance/skill-fast-loop-1789820305482/report.json`：PASS，sourceStable=true，22 个源文件指纹与交付前源码逐一一致。首次 15.858s / 5 主模型；自动 1.677s / 0 主模型、0 Jev |
| 最终源码：要求告诉查询结果 | 同份报告 outputBoundary=true；22.604s / 3 主模型，实际返回赵六／苏州；没有生成只会通用完成回执的候选 |
| 手动换材料、停止、改口 | `out/acceptance/skill-fast-loop-1789819853594/report.json`：全部 PASS，手动 1.549s / 0 主模型，默认材料未改；停止 .914s、停后无后续填表/点击；改口 27.031s / 6 主模型，结果正确。此报告在最终认证字段改版之前，控制与手动执行代码未再改；最终工程检查覆盖新版字段，不把该报告称为最终全源码复跑 |
| 更早的控制波动 | `.../skill-fast-loop-1789819217725/report.json`：PASS，改口 59.735s；原始 77.6s 亦保留。27.031s 单次结果不能证明慢问题已解决 |
| 真实输出契约判断 | `out/acceptance/skill-output-contract-1789819603324/report.json`：12/13，整体 FAIL；9 个额外交付全拒绝，普通短句 `查一下客户…` 得 .78、低于 .80，漏学。未降门限、未重复采样求绿 |
| 独立复核 | Sol 提出公开输入泄露、交付遗漏、模板吞额外动作；DeepSeek 独立复核补出虚构输出能力、概率越界、服务故障文案和旧认证绕过。主代理检查真实差异并修正；最终定点复核确认旧认证阻断关闭。收据 `/tmp/bys-skill-delegation/review-run/result.txt`；其当时“浏览器待跑”现由上述最终报告补齐 |

最终修正：

- 对外隐藏输入后，账本仍用宿主计算的真实 `valueHash`，避免把占位符当实际填写值；固定字面后缀也不能防止自由文本吞指令，最终只允许成对引号材料本地直达。复现及修复日志为 `primary-boundaries-before.log`、`primary-boundaries-after.log`（95 项）。
- Jev 判断必须如实描述“只回通用完成，不报告页面内容”。修正测试台曾把“告诉我结果”误标为纯流程的口径，旧数据保留；服务错误不算语义拒绝通过，非 0..1 概率直接拒绝，并区分无法核验与未覆盖。
- 自动资格使用 `learnedOutputContractVersion === SKILL_OUTPUT_CONTRACT_VERSION`，对所有来源生效；旧 boolean 及未认证演示不自动匹配，仍可显式手动运行。结构检查与输出资格分开，避免新候选产生之前就被版本门槛拦掉；修改内容删除认证，归档回退随内容恢复认证。含/不含 sourceRunId 两种旧认证回归都通过。
- 最终复跑主模型 8、Jev 3；本轮累计记录主模型 27、Jev 139，未追加 program-first 对照。预算限制已进入验收脚本，日常配置与数据未写入。

范围/维护：相对主线约 3000 行新增，主要为技能学习、路由、绑定执行、回归测试与验收器；生产代码把语义判断放到专门模块，session 负责接入既有生命周期。不存在可靠的“重复率为 0”量化结论，历史人工描述不能当指标。未引入运行时依赖，未改 package/lock。原路由业务正例超时回退、短句漏学、改口延迟、program-first 24.4% 未达40%和早期 trace 丢失未恢复均保留。
...
## 历史执行记录（各阶段快照，不代表当前状态）

以下保留原失败、worker 当时的环境限制和中间方案；当前判断、源码资格与最终证据以本文件上方为准。
...
说明：为避免反复采样求绿，全集合在拆分生效后各跑一次；`.85` 定为最终门限后**只补跑受影响组**。因此 holdout 全集合的 20/22 是 `clarify≥.9` 时的数字；当时唯一仍失败的缺材料样本正是受影响组里的 idx21 `按地区「苏州」查询客户`（`clarify=.89`），最终门限下未再整集合重采，不推算新的全集通过数；以受影响组实跑为准。唯一不受门限影响的失败是业务正例超时（见下）。
...
重复/无用代码与大函数：worker 当时人工查看新增模块，未指出具体重复或无用代码；未运行重复率/复杂度量化工具，不提供零重复率结论。
