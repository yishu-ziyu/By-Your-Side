...
> 历史交付记录（基线 `867f60b`）：后续独立 review 复现四个运行时反例，并发现本页旧验收器未真正检查原任务答案及重复执行。下列勾选和统计仅保留当时口径，不代表这些反例已通过；现行修复与证据见 [review 五项修复](20260918-jev-display-review-fixes.md)。原始报告不覆盖、不改判。
...
- [x] S1 运行中明确要求的宋体、双语、仅译文可由 Jev 判断，可同时指定字体和模式；缺失参数不猜默认值，判断模块不执行操作。
- [x] S2 命中的修改通过原工具应用于原任务页面；`runId` 不变、不新建任务、不提前结束原任务。
- [x] S3 原任务知道最新要求及该修改是否已执行并核验；后续不重复执行、不恢复旧设置，其余未完成要求保留。
- [x] S4 不确定/超时/混合要求回原路径；取消、接管、任务变化、同 URL 刷新后晚到候选不写入；连续修改同一属性后者覆盖，不同属性互不丢失。
- [x] S5 文字与语音复用同一条运行中显示修改路径，结果一致；语音不新增确认循环，不吞原任务播报。
- [x] S6 新能力独立开关默认关闭；关闭后行为不变；有可复现的真实浏览器配对证据与耗时/直达率。

开启新能力的性能门槛仍未确定；本轮只交付实现、测试与原始测量，开关保持关闭，等待独立 review 与门槛裁决。
...
脚本：`scripts/acceptance/jev-display-steering.mts`（隔离无头 Chrome for Testing + 真实构建扩展 + 生产 ConversationManager/runtime；端口占用时报告冲突，不终止任何进程）。

```bash
npx --no-install tsx scripts/acceptance/jev-display-steering.mts --headless [--smoke] [--boundaries-only]
```

执行模型：本机 `~/.sideagent/config.json` 的 `opencode-go/deepseek-flash`（可用 `SIDEAGENT_ACCEPTANCE_MODEL` 覆盖，不改日常配置）；Jev 固定 `jev-1.13.0` 只做路由。每对提交同一修改，关闭/开启 `displaySteerFastPath` 各一次（交替顺序），原任务为同一段真实读页+概括；记录页面结果、原任务是否同一 `runId` 正常交付、是否重复执行/重翻译。

两次完整真实运行的原始证据（均在 `out/acceptance/`，未剔除任何尝试）：

| 运行目录 | 配对 | 页面+原任务全对 | 关闭臂中位页面变更 | 开启臂中位页面变更 | 开启臂直达 | 成对中位差 | 边界失败 |
|---|---:|---:|---:|---:|---:|---:|---:|
| `jev-display-steering-1789718956959` | 10 | 20/20 | 7803.5 ms | 497.5 ms | 6/10 | -7119 ms | 1（问句歧义，见下） |
| `jev-display-steering-1789719609237` | 10 | 19/20 | 6697 ms | 818.5 ms | 6/10 | -5208 ms | 0 |

- 开启臂未直达的都是 Jev 回退（`direct_uncertain` / `timeout` / `font_uncertain` / `unsupported_original_font`），由原模型完成；两次运行无错页、无重复页面写入、无重新翻译，修改前后 `runId` 不变，原任务均正常交付。每次开启臂多一次 Jev 请求（新任务的既有判断 + 本次路由）。
- 一次真实失败：`1789719609237` 关闭臂「已有译文请只显示译文。」最终仍是双语，原模型交付却声称运行时已直接执行。这是关闭臂（原模型路径）的真实失败，保留在汇总里；同题开启臂正确。
- 边界（第二次运行）：局部标题不改整页、明确信息问句不改、纯否定不改、显示加另一步两项都完成、恢复网站字体，均通过。「你能把已有译文改成宋体吗？」按 Jev 既有政策（礼貌请求算请求）执行并核对，记为已记录的句类行为而不是失败。
- 语音路由不在本脚本覆盖：脚本只用真实侧栏文字入口；语音共用调度与播报由 `agent/test/voice-display-steering.test.ts` 覆盖，不把合成输入说成真人麦克风验收。取消/接管/刷新竞态由 Ticket 4 单测覆盖，新任务路径的取消/刷新已有 `jev-display-safety` 真实记录，本轮未重跑。
- 开关状态：脚本只在子进程内设置 `SIDEAGENT_DISPLAY_STEER_FASTPATH`，结束后清空；`~/.sideagent/config.json` 未改动，新能力保持关闭。性能门槛未定，本报告只给原始数据，不据此启用。
...
- 定点：13 个测试文件 199 项通过（判断、执行、运行中修改、连续修改/竞态、语音共用路径、连续插话、翻译中改口、调度去重）。
- `npm run check` 退出 1：模块边界 190 个生产文件通过、类型通过；单测 2030 通过、2 失败；后续阶段分别补跑：`npm run build` 通过；`npm run test:scale` 首跑 1 项 5 秒超时（20,000 条回执 12.2 秒），单独复跑 2/2 通过（7.6 秒）。
- 两处失败与本轮改动的关系：
  - `agent/test/tool-surface.test.ts` 系统提示词 15168 > 15000：既有失败，`agent/src/prompt.ts` 本轮未改，保留原门槛不删不降。
  - `agent/test/browser-program-binding.test.ts` QuickJS `InternalError: interrupted` 只出现在全量并行（47 秒）；单独复跑通过（3.1 秒），按环境负载波动记录，不改测试。

## 交付注记

- 起始 commit `089d9208e013ccfa04d8e71eca4b1f41555841a3`；六张票的本地提交依次为 `cf1839f`、`cc93504`、`4a89cf2`、`b7bdc85`、`1e1d428`、`47c8c73`、`0543f0c`（另 `e8f6c94` 为验收脚本模型覆盖）。
- 新增开关 `displaySteerFastPath` 固定在 `~/.sideagent/config.json` 可读；本轮结束时为未设置（默认关闭），未改动日常配置、未重载日常扩展、未发布。
- 未跑项：真人麦克风/TTS 听感；语音分类模型是否进一步跳过（需单独 spec）；性能门槛与启用裁决；Ticket 4 所述取消/刷新的本轮真实重跑（单测覆盖，新任务路径沿用既有 `jev-display-safety` 证据）。
