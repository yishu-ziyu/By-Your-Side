...
(globalThis as any).chrome = {debugger: {
  attach: async () => { attachCalls++; if (mockAttached) throw new Error('Another debugger is already attached'); mockAttached = true; await new Promise(resolve => setImmediate(resolve)); },
...
(globalThis as any).setTimeout = (...args: Parameters<typeof setTimeout>) => { const timer = realSetTimeout(...args); timers.push(timer); return timer; };
try {
