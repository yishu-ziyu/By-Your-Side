...
| 服务端断句延迟与不响应 | 无产品侧补救；验收用 `speechDelivered` 判音频是否送达，与断句结果分开记 | 2026-09-25 实测 StepFun 的 `speech_started` 比人声晚 2–10 秒；会话就绪后 1 秒内开口时，约四成服务端完全不回断句事件，约 60 秒后连接断开重连（不补发也会发生）。带停顿的句子会在逗号处被切成两轮，侧栏只显示后半句 |
