...
| 用户开口时旧回答停声（插话） | 侧栏在回答播放中按回声消除后的麦克风音量本地判定开口（约 -38 dBFS 以上累计 240 ms），立即停播并发 `barge_in`；宿主取消仍在生成的旧回复（`response.cancel`），旧回复迟到的音频不再下发。服务端先发 `speech_started` 时侧栏同样停播。不作废最新请求、不取消后台任务，与「停声」按钮不同 | StepFun 生成回答期间几乎不断句：实测 75 秒长回答里用户开口始终没有 `speech_started`，OS 项目日志也晚 4–5 秒，所以不能等服务端。取消生成后它有时 1 秒内开始听，有时 19–20 秒才生效或随后挂住，这时新问题丢失、由挂住检测提示重说。阈值未经真人外放校准，附和（“嗯”）也会掐断回答；见[插话验收](evals/20260925-voice-barge-in.md) |
...
| 语音面板的状态文字 | 就绪时显示「已连接，可以说了」，不露型号；连不上时只说「连不上语音服务」和去哪儿查（网络或设置页里的语音 key），底层原因只进诊断日志；缺 key 的提示由取 key 的一方给出，扩展里指向「更多 → 模型与语音」；「停声」只在正在回答时出现 | 浏览器的 WebSocket 错误不带原因，分不清是网络还是 key 被拒 |
