...
| 要操作页面却先读页 | 能直接操作页面时，`read_page` 的结果附 `next`：这里没有元素引用，要操作或圈画就调用 snapshot 后直接调用对应工具，只问内容就直接回答 | 通用指令要求读页后直接回答，实测模型读完页说「我先获取快照」就结束（同时段 5/7） |
