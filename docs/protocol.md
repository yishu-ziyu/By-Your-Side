...
`~/.sideagent/traces/*.jsonl` 按会话、任务、轮次与工具调用标识记录目标、模型、参数、返回、错误和耗时，以及接管/交还事件。日志写入失败不改变任务结果。只装扩展时，同样的行写进扩展的 IndexedDB（`sideagent-diagnostics`，格式由 `shared/run-trace-core.ts` 统一），同样只保留最近 20 个会话；设置页「诊断记录」可以导出成一个 jsonl 或清空。输入工具的填写值默认隐藏；图片只留元数据，其他文本按敏感字段与模式脱敏。任务和页面文本仍属于本地私密数据。
