...
| 范围 | 已知结论 | 尚缺什么／证据 |
|---|---|---|
| 回答交付（2026-09-24 架构修正） | 回答不再被核验扣下：模型正文直接交付，未完成项只做标注；闲聊、提问、读页、简单操作不再先列目标；写入前原文核对保留。日常 10 条底线 6/10 → 10/10，问答类中位 35.5 → 10.5 秒 | 侧栏杂项未清理；复制任务 155 秒；单次运行、未在日常 Chrome 验证；[开发日志与数据](devlog/20260924-02-回答不再被核验扣下.md)、[复制耗时调查](evals/20260924-copy-task-latency.md) |
...
1. **交互清理（当前重点）**：侧栏一个普通回合只留用户消息和回答；顶部任务卡、"查看任务回执"、"已完成 N 项"页脚、执行过程折叠等内部状态默认不出现，只在有副作用操作、失败或需要用户决定时出现。每次改动用 `scripts/acceptance/real-path/everyday-baseline.mts` 复测，数据追加到[开发日志](devlog/20260924-02-回答不再被核验扣下.md)的跟踪表。
