...
分发方向：先做 macOS，目标是只装扩展（见 [路线图](ROADMAP.md) 第 5 条）。扩展内 agent 实验（2026-09-24 已并入主目录，未提交）已证明两件事：扩展内 agent 能用用户套餐完成圈画；扩展能直连 StepFun 实时语音。下一步把语音搬进扩展。模型不再使用 cliproxy，改用用户套餐。

本轮不自动 commit、push、构建、重载或改日常配置。日常 dist 曾在源会话 14:15 重建，不能据此声称包含之后的点选改动；下次加载前重新核对版本。
